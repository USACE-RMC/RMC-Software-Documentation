...
def set_spans_from_grid(grid):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            if (
                (r > 0 and grid[r-1][c] is not None and grid[r-1][c]['text'] == cell['text'] and grid[r-1][c]['style'] == cell['style'])
                or
                (c > 0 and grid[r][c-1] is not None and grid[r][c-1]['text'] == cell['text'] and grid[r][c-1]['style'] == cell['style'])
            ):
                grid[r][c] = None
                continue
            # Count colspan
            ...
            for cc in range(c+1, n_cols):
                next_cell = grid[r][cc]
                if next_cell is not None and next_cell['text'] == cell['text'] and next_cell['style'] == cell['style']:
                    colspan += 1
                else:
                    break
            # Count rowspan
...
def extract_tables(doc):
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            if block.style.name == "RMC_Table Caption" and block.text.strip():
            ...
            first_row_header = all((cell is None or cell["style"] == "RMC_Table Header") for cell in first_row)
            first_col_header = all((cell is None or cell["style"] == "RMC_Table Header") for cell in first_col)
            if first_row_header and not first_col_header:
                ...
                for row in table_data:
                    if all(cell is None or cell["style"] == "RMC_Table Header" for cell in row):
                        ...
                        break
                headers = [
                    [cell_to_obj(cell) for cell in row]
                ...
                rows = [
                    [to_html_string(cell_to_obj(cell)) for cell in row[1:]]
                    for row in table_data
                ]
                data = rows
            else:
