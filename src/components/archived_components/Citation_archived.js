  ...
  useEffect(() => {
    const fetchBibData = async () => {
      try {
        ...
        const citationIndex = sortedCitations.findIndex((citation) => citation.citationKey === citationKey);
...
export const getUsedCitations = (docId) => Array.from(usedCitations.get(docId)?.values() || []);
