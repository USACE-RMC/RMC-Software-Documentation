...
<Equation equationKey="general-formula" equation="P_i = \frac{i - α}{n + 1 - 2α}" />
...
<Equation equationKey="non-parametric-distribution-1" equation="f(x) = \frac{p_{i+1} - p_i}{x_{i+1} - x_i}" />
...
<Equation equationKey="non-parametric-distribution-3" equation="F^{-1}(p) = x_i + (x_{i+1} - x_i) \left( \frac{p - p_i}{p_{i+1} - p_i} \right)" />
...
<Equation equationKey="likelihood-function" equation="L_S(D \mid \theta) = \prod_{i=1}^{n_s} f(X_i \mid \theta)" />
...
<Equation equationKey="log-likelihood-function" equation="L_L(D \mid \theta) = \prod_{i=1}^{n_L} \binom{h}{k} F(X_0 \mid \theta)^{h - k}" />
...
The binomial coefficient can be dropped from <EquationReference equationKey="log-likelihood-function" /> because it will be held constant
...
<Equation equationKey="combined-likelihood" equation="L(D \mid \theta) = L_S(D \mid \theta) \cdot L_L(D \mid \theta) \cdot L_I(D \mid \theta)" />
...
<Equation equationKey="AIC" equation="AIC = 2k - 2 \ln(\hat{L}) + \frac{2k^2 + 2k}{n - k - 1}" />

<Equation equationKey="BIC" equation="BIC = \ln(n)k - 2 \ln(\hat{L})" />
...
<Equation equationKey="RMSE" equation="RMSE = \sqrt{\frac{\sum_{i=1}^{n} (\hat{y}_i - y_i)^2}{n}}" />
...
shown in <FigReference figKey="create-new-bayesian" />. Next, give the Bayesian analysis a name and click **OK**.
...
<Equation equationKey="likelihood-data" equation="P(D \mid \theta) = \prod_{i=1}^{n} f(X_i \mid \theta)" />
...
<Equation equationKey="posterior-predictive" equation="F(X) = \int F(X \mid \theta, D) \cdot P(\theta \mid D) \, d\theta" />
...
<Figure figKey="histogram" src="figures/desktop-applications/rmc-bestfit/users-guide/v1.0/figure87.png" alt="Histogram." caption="Histogram." />
...
<Figure figKey="bivariate" src="figures/desktop-applications/rmc-bestfit/users-guide/v1.0/figure88.png" alt="Bivariate." caption="Bivariate." />
