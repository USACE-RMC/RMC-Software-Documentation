...
similar to non-cascading dam LifeSim modeling (see

<Link to="/docs/desktop-applications/lifesim/applications-guide/v1.0/estimating-consequences-for-dams">Estimating Consequences for Dams</Link> for
...
<Link to="/docs/desktop-applications/lifesim/applications-guide/v1.0/modeling-cascading-dam-failures-in-lifesim#purpose">Purpose</Link>). Similarly,
...
in the alternatives are relative to the hazard occurrence time selected for each hydraulic scenario. This is discussed in more detail later in
<Link to="/docs/desktop-applications/lifesim/applications-guide/v1.0/modeling-cascading-dam-failures-in-lifesim/#alternatives">Alternatives</Link>.
...
utilizes the NSI <Citation citationKey="NSI"/>. Reference
...
for additional information on the NSI and/or importing structure inventories into LifeSim. However, if the cascading dam failure scenarios are tacked
