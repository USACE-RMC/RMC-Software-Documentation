...
<ProcessList
  items={[
    ...
    { // STEP 8
      title: (
        <>
          <strong>Estimating Consequences Using Summary Grids</strong>, demonstrates how to estimate life loss and economic damages
          using hydraulic summary grid outputs, which differs from using Hierarchical Data Format (HDF) files from Hydrologic Engineering Center’s River Analysis System
          ...
          unsteady flow in HEC-RAS, and/or a study with limited output or information from the hydraulic model.
        </>
      ),
    },
  ]}
