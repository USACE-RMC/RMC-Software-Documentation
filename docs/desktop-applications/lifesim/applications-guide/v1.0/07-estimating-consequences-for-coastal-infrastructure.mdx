...
location where levee elevations are higher. Breach Locations 2 and 3 breach the proposed buried seawall and were selected by the USACE New York District. 
...
available to the user when HDF files are utilized. The terrain HDF file and all associated terrain files are also required to import hydrualic data.
...
to best establish the Hazard Occurrence time in LifeSim. For levees in a coastal setting, the team must be able to distinguish the source of water (i.e., breach flow, 
...
floodwall breach, the time of wind/wave-overtopping will be set as the Hazard Occurrence time in LifeSim. ***This should be discussed at length with the rest of the team, 
...
Gaining an understanding of how flood depths and flood arrival times interact with each other and structures within the leveed area is a good place to
 ...
 up the SSSI LCL 1ft OT breach inundation area by depth and arrival time, respectively.
...
and <FigReference figKey="figure-169"/>, again, sort by ‘Life_Loss_Total_Mean.’ Compare maximum depths and velocities to each structure’s stability criteria (e.g., wood-anchored,
