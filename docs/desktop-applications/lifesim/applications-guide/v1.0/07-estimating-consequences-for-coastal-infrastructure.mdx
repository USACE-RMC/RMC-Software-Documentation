...
shelter-in-place action to those who were unable to or chose not to evacuate well before the non-evacuation depth (i.e., 2-foot flood depths) occurs.
...
Refer to <Link to="/docs/desktop-applications/lifesim/applications-guide/v1.0/estimating-consequences-for-levees-and-floodwalls/#emergency-planning-zones">Estimating
Consequences for Levees and Floodwalls, Emergency Planning Zones</Link> if using existing consequences elicitation data, the Levee Safety Tool, or
...
For example, regarding to the SSSI coastal storm study, the New York City (NYC) Emergency Management website provides detailed information on coastal storms and
...
you attempt understand who is receiving the order, what they are being told to do, and when they will be told to do it. ‘Know Your Zone' (often referred
...
are additional considerations when creating alternatives for a coastal model. Much like riverine levees, the PAR may be subjectec to a warning and delay continuum.
...
or emergency managers have wait to warn the impacted population (hazard identification between 3 hours prior to its occurrence and 30 minutes after).
In the example below, this warning may be appropriate for a 0.5 Annual Exceedance Probability (AEP) event as this is a frequent event and has the
potential to impact very few people. It's possible the “warning” would only be based on the population self-warning relative to when they see
floodwaters.
...
depths exceed 4ft, which could result in life threatening flooding. It's possible that even this frequent of an event would be forecasted in
advance.
...
It is not uncommon for evacuation orders to be issues days in advance of an event in a coastal environment. <FigReference figKey="figure-165"/> shows a far more optimistic, or “optimal”,
