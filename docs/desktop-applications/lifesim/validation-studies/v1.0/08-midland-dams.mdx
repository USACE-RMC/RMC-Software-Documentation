...
The model’s life loss (compared to the real event’s zero fatalities) is primarily located in hot spots shown in <FigReference figKey="midland-lifesim-map"/>.
  ...
  [
    {
      title:[
        <>
        "Sturgeon Creek Area" (<FigReference figKey="midland-lifesim-map"/>): W Main St and an adjacent neighborhood northeast of Northwood University.
        </>
      ]
    },
    {
      title:[
        <>
        "Poseyville Dike Area" (<FigReference figKey="midland-lifesim-map"/>): Industrial area along Venture Dr and S Poseyville Rd.
        </>
      ]
    }
  ]
