...
For more information on the quantitative risk analysis framework in RMC-TotalRisk, refer to the technical reference manual (Smith et al., 2025) <Citation citationKey="TechRef" />.

## Create New Risk Analysis

To create a risk analysis, right-click on the **Risk Analyses** folder in the Project Explorer (<FigReference figKey="figure-123" />) or navigate to
...
(<FigReference figKey="figure-124" />). In the Properties window, you can configure the name, description, consequence type, and consequence
...
You can access additional risk analysis options through the Options tab in the Properties window (<FigReference figKey="figure-124" />). For
...
The simulation options allow you to configure settings for the uncertainty analysis and the loss exceedance curve (LEC) results (<FigReference figKey="figure-125" />).
...
integration. <FigReference figKey="figure-126" /> shows the available options for the ASR method.
...
handles numerical integration. <FigReference figKey="figure-127" /> shows the available options for the VEGAS method.
...
For more details on the numerical integration methods used by RMC-TotalRisk, refer to the technical reference manual (Smith et al., 2025) <Citation citationKey="TechRef" />.

## System Component Options

<FigReference figKey="figure-128" /> shows the system component options.
...
The system options define how the system components interact in the risk calculations (<FigReference figKey="figure-129" />).
...
rather than individual components. <FigReference figKey="figure-130" /> displays the risk measure options.
...
For more details about the additional risk measures in RMC-TotalRisk, refer to the technical reference manual (Smith et al., 2025) <Citation citationKey="TechRef" />.

## Risk Diagram

RMC-TotalRisk defines a risk analysis using an intuitive diagram, as shown in <FigReference figKey="figure-131" />. This diagram provides a clear way to create and
...
<FigReference figKey="figure-131" /> illustrates a single system component for a dam safety risk analysis. The diagram includes a non-failure mode, represented 
...
1. **Spillway erosion failure mode (PFM 1)**: This mode, shown in the center of the diagram, connects the hazard function at Dam A to the PFM 1 response function
...
You can add nodes to the risk diagram in three ways (<FigReference figKey="figure-133" />):
...
(<FigReference figKey="figure-134" />).
...
<FigReference figKey="figure-135" /> illustrates a levee risk analysis. The hazard function is a river peak flow frequency curve, transformed into a peak river 
...
(<FigReference figKey="figure-136" />):
...
<FigReference figKey="figure-137" /> illustrates the overall risk and Monte Carlo simulation framework used in RMC-TotalRisk. The **Mean Risk Only**
...
manual (Smith et al., 2025) <Citation citationKey="TechRef" />.
