...
<FigReference figKey="figure-145"/> shows an example of the integration diagnostics for the estimated integration standard error during the simulation. In 
...
For mathematical details on these risk measures, refer to the technical reference manual (Smith et al., 2025) <Citation citationKey="TechRef" />.

As shown in <FigReference figKey="figure-146" />, you can select the desired risk type and risk measure from the dropdown menus to view the results.
...
for the distribution appear in the upper right-hand corner (<FigReference figKey="figure-147" />).
...
<FigReference figKey="figure-148" /> shows the hazard exceedance probability for the background risk at Dam A. Since background risk represents the 
...
<FigReference figKey="figure-149" /> displays the hazard exceedance probability for the excess risk at Dam A. The profile shows an inflection point near a 
...
National Flood Insurance Program (NFIP) for levees. For more details, refer to the technical reference manual (Smith et al., 2025) <Citation citationKey="TechRef" />.

<FigReference figKey="figure-150" /> illustrates an example risk diagram for a levee risk analysis. The primary hazard is river peak flow frequency, which 
...
in <FigReference figKey="figure-151" />.
...
<FigReference figKey="figure-152" /> shows the standard cumulative distribution plot for assurance in the levee risk analysis example. The mean API is
...
85%, the levee accreditation recommendation must be supported by uncertainty analysis, past system performance, and other factors (USACE, 2019) <Citation citationKey="ECB2019" />.

## Tornado Plot

A **tornado plot** (<FigReference figKey="figure-153" />) visually illustrates the sensitivity of risk results to input functions at each hazard level. The plot
...
In the dam risk example (<FigReference figKey="figure-154" />), the X-Y plot reveals a strong correlation between the excess risk of failure mode PFM 2 (concentrated
...
mode correspond to substantial changes in overall system risk. These findings are consistent with the sensitivity results shown in <FigReference figKey="figure-153" />.
...
Tabular results display data based on the selected risk type and risk measure (<FigReference figKey="figure-155" />). The table includes a column for each
...
statistics for the selected column, as shown in <FigReference figKey="figure-156" />.
