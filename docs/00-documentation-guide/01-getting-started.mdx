...
<Tabs>
    ...
    **✅ Clone the Repository**
    ...
    If you've forked the repository to your own GitHub account, replace `USACE-RMC` with your GitHub username.
...
To generate the optimized static files for deployment, run:
...
**Deployment is an administrator-level task.** Contributors should focus on creating and editing documentation content. Site administrators will handle deployment to production.
...
- Try opening a new Command Prompt window with administrator privileges
...
- On USG computers, verify you have network access and proxy settings are correct
- Try running Command Prompt as administrator
- Clear npm cache: `npm cache clean --force`, then try again

### Permission Errors on USG Computers

**Problem:** Installation fails with permission or access denied errors.

**Solutions:**
- Install Node.js in a user-accessible location like `C:\Portable Software\` (not `Program Files`)
- Use the "Edit environment variables **for your account**" option, NOT system environment variables
- Request assistance from CIO-G6 if persistent permission issues occur
- Ensure antivirus/security software isn't blocking the installation
