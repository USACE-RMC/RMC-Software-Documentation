...
To create a new `.mdx` file in Visual Studio Code, right-click on the folder where the file will be located and click "New File". Name the file with your desired name and add `.mdx` for the document type. Follow the naming and numbering convention outlined in the [`docs/`](./04-docs-folder.mdx) section.
...
React components available within the RMC Software Documentation project are outlined in the [React Components](./11-react-components.mdx) section.
