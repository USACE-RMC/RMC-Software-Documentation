...
2. Encounters first `<Figure figureKey="fig-workflow">` → assigns Figure 1
3. Encounters second `<Figure figureKey="fig-results">` → assigns Figure 2
...
```bash
npm run counters    # Regenerate figure/table/equation numbers
npm run sidebars    # Regenerate sidebar navigation
npm run versions    # Regenerate version data
npm run report-map  # Regenerate report ID mappings
```

**When to run manually:**
- Troubleshooting numbering issues
- Sidebar not updating after structural changes
...
1. Check that each figure has a unique `figureKey`
2. Verify files are numbered correctly (`01-`, `02-`, `03-`)
3. Run `npm run counters` to regenerate
4. Refresh browser
...
**Use selective script execution:**
```bash
# Only regenerate counters
npm run counters && npm start

# Only regenerate sidebars
npm run sidebars && npm start
```
...
- **Convert Word documents:** [DOCX Converter](./04-docx-converter.mdx)
