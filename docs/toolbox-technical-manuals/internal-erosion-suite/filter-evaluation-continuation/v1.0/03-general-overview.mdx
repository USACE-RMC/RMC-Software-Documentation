...
message bar will vary depending on the computer’s settings and installed add-ins. <FigReference figKey="enable-macros"></FigReference> displays examples of different wordings that may appear in the message bar.
...
title in case multiple copies of the worksheet are created for different analysis scenarios (<FigReference figKey="heading"></FigReference>).
...
a user-specified datum along with feet (e.g., ft-MSL [Mean Sea Level]). <FigReference figKey="NAVD88"></FigReference>, <FigReference figKey="NGVD29"></FigReference>, and <FigReference figKey="datum"></FigReference> illustrate
...
Most calculation worksheets break down complex analysis into computational steps following a logical sequence (<FigReference figKey="step-banner"></FigReference>). Some simpler
...
are labeled as options (<FigReference figKey="option-banner"></FigReference>).
