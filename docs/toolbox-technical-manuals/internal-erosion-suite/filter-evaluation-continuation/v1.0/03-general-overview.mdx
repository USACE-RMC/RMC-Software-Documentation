...
message bar will vary depending on the computer’s settings and installed add-ins. <FigReference figKey="figure-1"/> displays examples of different wordings that may appear in the message bar.
...
title in case multiple copies of the worksheet are created for different analysis scenarios (<FigReference figKey="figure-2"/>).
...
a user-specified datum along with feet (e.g., ft-MSL [Mean Sea Level]). <FigReference figKey="figure-3"/>, <FigReference figKey="figure-4"/>, and <FigReference figKey="figure-5"/> illustrate
...
Most calculation worksheets break down complex analysis into computational steps following a logical sequence (<FigReference figKey="figure-6"/>). Some simpler
...
are labeled as options (<FigReference figKey="figure-7"/>).
