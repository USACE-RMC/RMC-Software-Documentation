...
al. (2011) <Citation citationKey="Sellmeijer2011"/>. The adjusted calculation rule extended and updated the piping model
...
piping model testing parameters shown in <TableReference tableKey="table-2"/>.
...
sand (_θ_), and White’s constant (_η_) as illustrated in <FigReference figKey="figure-35"/>. The bedding angle and White’s
...
al. (2009) <Citation citationKey="vanBeek2009"/> provides guidance for visual estimation of _KAS_ as illustrated
in <FigReference figKey="figure-36"/>.
...
For deterministic analysis, input only the most likely values. The mean values used for subsequent calculations are the most likely (or mode)
values. <FigReference figKey="figure-37"/> illustrates the deterministic input.
...
values. <FigReference figKey="figure-38"/> illustrates the probabilistic input without using @RISK.
...
column. <FigReference figKey="figure-39"/> illustrates the probabilistic input using @RISK.
...
The intrinsic permeability of the fine sand (piping) layer is calculated using <EquationReference equationKey="equation-16" />.
...
background. Van Beek et al. (2012) <Citation citationKey="vanBeek2012" /> conducted piping experiments on multi-layer sand
...
horizontal permeability of the fine sand (piping) layer, shown in <FigReference figKey="figure-40"/>,
using <EquationReference equationKey="equation-17" />.
...
> _k<sub>h,f</sub>_ = permeability of the fine (piping) layer in the horizontal direction (centimeter/second)  
...
The average intrinsic permeability is calculated using <EquationReference equationKey="equation-18" />.
...
calculations is the most likely (or mode) value. <FigReference figKey="figure-41"/> illustrates the deterministic
...
values used for subsequent calculations are the most likely (or mode) values. <FigReference figKey="figure-42"/>
...
the minimum, most likely, and maximum values. <FigReference figKey="figure-43"/> illustrates the probabilistic
...
<FigReference figKey="figure-44" /> illustrates
...
<Figure
  ...
  caption="Step 3 of Sellmeijer worksheet: Probabilistic input for multi-layer aquifer without using @RISK."
></Figure>
...
distribution entered in the third column. <FigReference figKey="figure-45"/> illustrates the
...
are the mean for the @RISK distribution entered in the third column. <FigReference figKey="figure-46"/>
...
<Figure
  ...
  caption="Step 3 of Sellmeijer worksheet: Probabilistic input for multi-layer aquifer using @RISK."
></Figure>

## Field Critical Horizontal Gradient for BEP Progression

Step 4 calculates the field critical horizontal gradient for BEP progression as illustrated in <FigReference figKey="figure-47"/>.

<EquationReference equationKey="equation-19" />, for the critical horizontal gradient
...
coefficient of uniformity (_U_) and is calculated using <EquationReference equationKey="equation-20" />.
...
Since it is easier to estimate the specific gravity of soil particles than the submerged unit weight of soil particles, the substitution in <EquationReference equationKey="equation-21" />
is made to the equation for _F<sub>R</sub>_.

<Equation equationKey="equation-21" equation="\frac{γ'_p}{γ_w} = G_s - 1" />
...
Van Beek et al. (2010) <Citation citationKey="vanBeek2010" /> indicate that _KAS_ and _U_ appear less important than other
...
terms from the equation for _F<sub>R</sub>_ as illustrated in <FigReference figKey="figure-48"/>.
...
and the intrinsic permeability of the piping layer (_κ_) and is calculated using <EquationReference equationKey="equation-22" />.
...
{" "}<EquationNoRef equation="\left(\frac{D}{L}\right)" inline={true}/> and is calculated using <EquationReference equationKey="equation-23" />.
...
the model test parameters. Van Beek et al. (2015) <Citation citationKey="vanBeek2015" /> found that 3D configurations
...
two scenarios are illustrated in <FigReference figKey="figure-49"/>
and <FigReference figKey="figure-50"/>.
...
<Equation
  ...
  equation="i_{avf} = \frac{\Delta H}{L} = \frac{HW - TW}{L}"
/>
...
<Equation equationKey="equation-15" equation="FS = \frac{i_{ch}}{i_{avf}}" />
...
For deterministic and probabilistic analyses, cells with _FS_ less than 1 have an orange background. <FigReference figKey="figure-51"/>
illustrates the deterministic tabular output, and <FigReference figKey="figure-52"/> illustrates the
...
(red shaded line). <FigReference figKey="figure-53"/> illustrates the deterministic graphical output.
...
<FigReference figKey="figure-54" /> illustrates the plot
...
background for deterministic analysis. <FigReference figKey="figure-55"/> illustrates the graphical output
...
<FigReference figKey="figure-56" /> illustrates the plot
