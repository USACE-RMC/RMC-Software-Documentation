...
<EquationReference equationKey="i-pmt" />
can be used along with the correction factors to assess the FS for BEP progression in the field.
While this figure is quite useful, it is difficult to estimate the uncertainty in the critical point
gradient because study averages were presented. For example, one of the single data points is the
