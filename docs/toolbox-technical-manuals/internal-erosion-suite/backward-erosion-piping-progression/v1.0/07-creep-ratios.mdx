...
etc. as illustrated in <FigReference figKey="figure-57"/>.
...
Step 1 calculates the line of creep (seepage path length) as illustrated in <FigReference figKey="figure-58"/>. The
...
The line of creep (_L_) for Bligh’s method is calculated using <EquationReference equationKey="equation-24" />. The
weighted line of creep for Lane’s method (_L<sub>W</sub>_) is calculated using <EquationReference equationKey="equation-25" />.

<Equation equationKey="equation-24" equation="L = L_1 + W + L_2 + 2D" />

<Equation equationKey="equation-25" equation="L_W = \frac{L_1 + W + L_2}{3} + 2D" />
...
in <FigReference figKey="figure-59"/>. Use the drop-down list to select the material that best represents
...
field horizontal critical gradient is calculated using <EquationReference equationKey="equation-26" />
and <EquationReference equationKey="equation-27" /> for Bligh’s method and Lane’s method, respectively. For _D_ greater
than zero, “N/A” displays for _i<sub>ch</sub>_.

<Equation equationKey="equation-26" equation="i_{ch} = \frac{1}{C_{min}}" />

<Equation equationKey="equation-27" equation="i_{ch} = \frac{1}{C_{W,min}}" />
...
consideration. The creep ratio for Bligh’s method (_C_) is calculated using <EquationReference equationKey="equation-28" />.
The weighted creep ratio for Lane’s method (_C<sub>W</sub>_) is calculated using <EquationReference equationKey="equation-29" />.

<Equation equationKey="equation-28" equation="C = \frac{L}{ΔH} = \frac{L}{HW-TW}" />
...
minimum value have an orange background. <FigReference figKey="figure-60"/> illustrates the
tabular summary.
...
(or safe) creep ratios for each method. <FigReference figKey="figure-61"/> illustrates the
...
<FigReference figKey="figure-62" /> illustrates the plot options
...
illustrated in <FigReference figKey="figure-63"/>. For example, if only the results from the Bligh’s
