...
al. <Citation citationKey="Sellmeijer2011" bibFile="/bibliographies/108-bib.json"/>. The adjusted calculation rule extended and updated the piping model
...
piping model testing parameters shown in <TableReference parentDocId="108" tableKey="sellmeijer-parameter-limits"/>.
...
sand (_θ_), and White’s constant (_η_) as illustrated in <FigReference parentDocId="108" figKey="sellmeijer-discrete-input"/>. The bedding angle and White’s
...
al. <Citation citationKey="vanBeek2009" bibFile="/bibliographies/108-bib.json"/> provides guidance for visual estimation of KAS as illustrated
in <FigReference parentDocId="108" figKey="sellmeijer-KAS-estimate"/>.
...
values. <FigReference parentDocId="108" figKey="sellmeijer-deterministic-input"/> illustrates the deterministic input.
...
values. <FigReference parentDocId="108" figKey="sellmeijer-probabilistic-input-no-risk"/> illustrates the probabilistic input without using @RISK.
...
column. <FigReference parentDocId="108" figKey="sellmeijer-probabilistic-input-risk"/> illustrates the probabilistic input using @RISK.
...
The intrinsic permeability of the fine sand (piping) layer is calculated using Equation 16.

$$
K_{f} = k_{h,f} \frac{μ}{γ_w} \tag{16}
$$
...
background. Van Beek et al. <Citation citationKey="vanBeek2013" bibFile="/bibliographies/108-bib.json"/> conducted piping experiments on multi-layer sand
...
horizontal permeability of the fine sand (piping) layer, shown in <FigReference parentDocId="108" figKey="sellmeijer-multi-layer-fndn"/>, using Equation 17.
...
$$
k_{h,avg} = \frac{k_{h,f}D_f + k_{h,c}D_{c}}{D} \tag{17}
$$
...
The average intrinsic permeability is calculated using Equation 18.

$$
k_{avg} = k_{h,avg} \left(\frac{μ}{γ_w}\right) \tag{18}
$$
...
calculations is the most likely (or mode) value. <FigReference parentDocId="108" figKey="sellmeijer-single-layer-deterministic"/> illustrates the deterministic
...
values used for subsequent calculations are the most likely (or mode) values. <FigReference parentDocId="108" figKey="sellmeijer-multi-layer-deterministic"/>
...
the minimum, most likely, and maximum values. <FigReference parentDocId="108" figKey="sellmeijer-single-layer-probabilistic-no-risk"/> illustrates the probabilistic
...
<FigReference parentDocId="108" figKey="sellmeijer-multi-layer-probabilistic-no-risk"/> illustrates the probabilistic input for multi-layer aquifer without using @RISK.
...
distribution entered in the third column. <FigReference parentDocId="108" figKey="sellmeijer-single-layer-probabilistic-risk"/> illustrates the
...
are the mean for the @RISK distribution entered in the third column. <FigReference parentDocId="108" figKey="sellmeijer-multi-layer-probabilistic-risk"/>
...
Step 4 calculates the field critical horizontal gradient for BEP progression as illustrated in <FigReference parentDocId="108" figKey="sellmeijer-critical-gradient-terms"/>.
Equation 19, for the critical horizontal gradient for BEP progression (_i<sub>ch</sub>_), is composed of three terms: a resistance factor that accounts
for the strength of the layer subject to BEP; a scale factor that relates pore size and seepage size; and a geometrical shape factor.

$$
i_{ch} = \left(F_R\right) \left(F_S\right) \left(F_G\right) \tag{19}
$$
...
coefficient of uniformity (_U_) and is calculated using Equation 20.

$$
F_R = η \left(\frac{γ'_p}{γ_w}\right) \tanθ \left(\frac{RD}{72.5}\right)^{0.35} \left(\frac{U}{1.81}\right)^{0.13} \left(\frac{KAS}{49.2}\right)^{-0.02} \tag{20}
$$
...
Since it is easier to estimate the specific gravity of soil particles than the submerged unit weight of soil particles, the substitution in Equation 21
is made to the equation for _F<sub>R</sub>_.

$$
\frac{γ'_p}{γ_w} = G_s - 1 \tag{21}
$$
...
Van Beek et al. <Citation citationKey="vanBeek2010" bibFile="/bibliographies/108-bib.json"/> indicate that _KAS_ and _U_ appear less important than other
...
terms from the equation for FR as illustrated in <FigReference parentDocId="108" figKey="sellmeijer-critical-gradient-terms-no-u-kas"/>.
...
and the intrinsic permeability of the piping layer (_κ_) and is calculated using Equation 22.

$$
F_S = \left(\frac{d_{70}}{κL}\right)^{\frac{1}{3}} \left(\frac{0.000208}{d_{70}}\right)^{0.6} \tag{22}
$$
...
$$
F_G = 0.91 \left(\frac{D}{L}\right)^{\frac{0.28}{\left(\frac{D}{L}\right)^{2.8} - 1} + 0.04} \tag{23}
$$
...
the model test parameters. Van Beek et al. <Citation citationKey="vanBeek2015" bibFile="/bibliographies/108-bib.json"/> found that 3D configurations
...
two scenarios are illustrated in <FigReference parentDocId="108" figKey="sellmeijer-field-gradient-reduction"/>
and <FigReference parentDocId="108" figKey="sellmeijer-field-gradient-no-reduction"/>.
...
Step 5 calculates the average hydraulic gradient in the foundation by dividing the net hydraulic head by the seepage path length using Equation 24.

$$
i_{avf} = \frac{∆H}{L} = \frac{HW - TW}{L} \tag{24}
$$

where:

> **∆H** = net hydraulic head (feet)  
> **HW** = headwater level (feet)  
> **TW** = tailwater level (feet)  
> **L** = seepage path length (feet)

The FS against BEP progression is calculated using Equation 25.

$$
FS = \frac{i_{ch}}{i_{avf}} \tag{25}
$$

where:

> **i\_{ch}** = critical horizontal gradient for BEP progression  
> **i\_{avf}** = average horizontal gradient in the foundation at the pipe head

For deterministic analysis, the **FS** is calculated using the most likely values of the random variables and is summarized in a table. Cells that do
not apply have a gray background.

For probabilistic analysis, the **FS** is calculated as described for the deterministic analysis but for the mean values of the random variables, and
...
that resulted in an **FS** less than 1 \[P(FS < 1)\].
...
For deterministic and probabilistic analyses, cells with **FS** less than 1 have an orange background. <FigReference parentDocId="108" figKey="sellmeijer-deterministic-tabular-output"/>
illustrates the deterministic tabular output, and <FigReference parentDocId="108" figKey="sellmeijer-probabilistic-tabular-output"/> illustrates the
...
(red shaded line). <FigReference parentDocId="108" figKey="sellmeijer-deterministic-graphical-output"/> illustrates the deterministic graphical output.
...
<FigReference parentDocId="108" figKey="sellmeijer-deterministic-plot-options"/> illustrates the plot options for this chart. The maximum value for the 
...
background for deterministic analysis. <FigReference parentDocId="108" figKey="sellmeijer-probabilistic-graphical-output"/> illustrates the graphical output
...
<FigReference parentDocId="108" figKey="sellmeijer-probabilistic-plot-options"/> illustrates the plot options for this chart. The vertical reference 
