...
├── pages/             → Custom web pages (homepage, landing pages)
├── theme/             → Docusaurus theme overrides
├── docConfig.js       → Central document active/draft registry
├── draftDocs.js       → Thin shim around docConfig.js
├── imageDimensions.js → Auto-generated image metadata
...
- `EventTreeTOC.js` - Event tree table of contents
- `Counters.js` - Build-time counter utility
- `ModuleMap.js` - Module diagram rendering
- `DocChapterNav.js` - Chapter-level navigation

**Landing Page Components:**

- `ContentBox.js` / `ContentBoxLarge.js` - Dashboard cards
- `ContentBubble.js` / `ContentBubbleLarge.js` - Dashboard card sub-components
- `ContentCard.js` - Content card layout

**Site Features:**

- `SiteTour.js` - Guided site tour overlay
- `TourButton.js` - Tour launch button
- `VersionNotice.js` - Version mismatch notice (used internally by NavContainer)

**Schema Components (DST Database):**

- `SchemaGroup.js` - Schema group container
- `SchemaTable.js` - Database schema table
- `SchemaTableCompact.js` - Compact schema table variant
- `SchemaTag.js` - Schema tag labels
- `SchemaRelationshipDiagram.js` - Entity relationship diagrams

**Icon Components (`src/components/icons/`):**

19 SVG icon components for landing pages and navigation: `CodeIcon`, `DesktopAppIcon`, `DocumentationGuideIcon`, `DSTDatabaseIcon`, `DSTIcon`, `GitHubIcon`, `InternalErosionIcon`, `LSTIcon`, `OvertoppingIcon`, `RiskCalculationsIcon`, `RiverineErosionIcon`, `RRFTIcon`, `SeismicHazardIcon`, `SparkleIcon`, `SpillwayErosionIcon`, `StructuralIcon`, `StyleGuideIcon`, `ToolboxIcon`, `WebAppIcon`
...
**Purpose:** Manages state for the guided site tour feature (SiteTour component).

**Provides:**

- Tour active/inactive state
- Current tour step tracking
- Tour start/stop/navigation functions

**Used By:** `SiteTour.js`, `TourButton.js`, and `Root.js` theme wrapper.

---

## Client Modules Directory

**Location:** `src/clientModules/`

Client modules run on every page load and are registered in `docusaurus.config.js`.

### analyticsEvents.js

**Purpose:** Sends custom Google Analytics 4 events for page views, search interactions, and tour tracking.

### gtagPolyfill.js

**Purpose:** Provides a polyfill for the `gtag()` function to prevent errors when the Google Analytics script hasn't loaded yet.

### scrollToAnchor.js

**Purpose:** Handles smooth scrolling to anchor links on page navigation, ensuring hash-based navigation works correctly.
...
Global styles for DST database schema components:

- Schema table and group layout
- Schema tag colors and sizing
- Relationship diagram styling

**chapter-nav.css**

Styles for the `DocChapterNav` chapter navigation component:
...
- `eventTreeToc.json` — Table of contents data for the Typical Event Tree Database
- `dstSchemaData.js` — Schema metadata for DST database documentation

**Generated By:** `scripts/generateEventTreeTOC.js` and build-time data extraction

**Used By:** `EventTreeTOC` and schema components for rendering structured data
...
Additional pages for organizing documentation by category:

**Category Hub Pages:**

- `desktop-applications.js` - Hub page for all desktop applications
- `toolboxes.js` - Hub page for all toolbox technical manuals
- `web-applications.js` - Hub page for all web applications
- `changelog.js` - Site changelog page
- `feedback.js` - User feedback form page

**Application-Specific Pages (`desktop-applications/`):**

- `lifesim.js`, `rmc-bestfit.js`, `rmc-rfa.js`, `rmc-totalrisk.js`

**Toolbox-Specific Pages (`toolboxes/`):**

- `internal-erosion-suite.js`, `overtopping-suite.js`, `risk-calculations-suite.js`, `seismic-hazard-suite.js`

**Web Application Pages (`web-applications/`):**

- `dst.js`, `lst.js`, `rrft.js`

**Developer Pages (`docs/`):**

- `dev.js` - Developer resources landing page
...
**Purpose:** Wraps the entire application with context providers (ReportIdContext, TourContext) and global components (SiteTour).

---

**Layout Override**

**Files:** `src/theme/Layout/index.jsx` and supporting modules:

- `buildNavLinks.js` — Constructs navigation link data
- `externalLinks.js` — External link configuration
- `ThemeToggle.jsx` — Light/dark theme toggle component
- `usaceLinks.js` — USACE-specific navigation links
- `useHeaderOffsets.js` — Header offset calculation hook
- `useLatestVersions.js` — Latest version resolution hook

**Purpose:** Custom layout wrapper that adds USACE branding, navigation links, and version-aware header behavior.

---

**NotFound Override**

**File:** `src/theme/NotFound/Content/index.js`

**Purpose:** Custom 404 page content with site-specific messaging and navigation.

---

**DocItem Theme Component**

**Files:** `src/theme/DocItem/index.js`, `src/theme/DocItem/Layout/index.js`, `src/theme/DocItem/Layout/styles.module.css`

**Purpose:** Override the default documentation page wrapper to add a version-aware DRAFT watermark and custom layout styles.

**Custom Functionality:**

- Reads the current page's pathname and matches it against the `draftDocs` list (which is derived from `src/docConfig.js`)
- Cross-references the version in the URL against `latestVersions.json` so the watermark only renders on the **latest** version of a flagged document — older versions of the same document stay unwatermarked
...
- See [Appendix C: Search Configuration](./18-appendix-c-search-configuration.mdx)
...
## docConfig.js and draftDocs.js

**Location:** `src/docConfig.js` and `src/draftDocs.js`

`src/docConfig.js` is the **central source of truth** for which documents are published and which display the DRAFT watermark. Every published document tile has exactly one entry in the `docs` array with two orthogonal flags:
...
```javascript
{
  ...
  active: true,    // controls build inclusion, sidebar, and search
  draft: false,    // controls watermark only
},
```

The file is CommonJS so both the React app (for landing-page tile rendering) and Node build scripts (for sidebar generation, version metadata, build exclusion) can consume it from the same source. It also exports three helper functions:

- `filterByCategoryAndSoftware(category, software)` — used by landing pages to filter the doc list
- `getInactiveDocLocations()` / `getDraftDocLocations()` — used by build scripts
- `shouldExcludeFromBuild(docsRelativePath)` — used by `scripts/generateSidebars.js` and `scripts/versions.js` to skip inactive docs in prod builds

**Active vs. draft semantics:**

- `active: true` → doc is built, served, sidebar-registered, and indexed by the search crawler.
- `active: false` → doc is excluded from the prod build (no static HTML, no sidebar, no Algolia entry); landing page renders a "Coming Soon" tile. In dev (`DOCS_MODE !== 'prod'`), the doc is built so contributors can work on it without exposing it publicly.
- `draft: true` → DRAFT watermark renders on the **latest version** of the doc only.
- `draft: false` → no watermark.

The two flags are orthogonal. A doc may be `active+draft` (published with watermark, fully searchable) or `inactive+draft` (unpublished, watermark configured for whenever it goes active).

**Download-only entries** — entries with a `downloadUrl` but no `doc_location` — link directly to a PDF and are unaffected by the build-inclusion logic.

**`src/draftDocs.js`** is a thin shim that imports `getDraftDocLocations()` and exports the resulting array of doc locations. It exists because `src/theme/DocItem/index.js` needs a clean ESM-style import for the client-side watermark check, while `docConfig.js` is consumed via `require()` from build scripts. Concretely:
...
Edit `src/docConfig.js` directly — flip `active` or `draft` on the relevant entry, or add a new entry for a new document. The file is checked into version control and is not auto-generated.
...
- `components/` - React components for use in MDX (including `icons/` subdirectory)
- `contexts/` - React context providers (ReportIdContext, TourContext)
- `clientModules/` - Client-side modules (analytics, scroll behavior)
- `css/` - Global stylesheets
- `data/` - Build-generated data files
- `pages/` - Custom web pages and landing pages
- `theme/` - Docusaurus theme overrides (Root, Layout, DocItem, NotFound, SearchBar)

**Auto-Generated Files:**

- `reportIdMap.js` - Document ID mappings
- `imageDimensions.js` - Image dimension metadata

**Contributor-Edited Files:**

- `docConfig.js` - Central registry of active/draft flags for every document tile
- `draftDocs.js` - Thin shim around `docConfig.js` consumed by the DocItem theme component
...
- Build process: [Appendix B: Build Process Overview](./17-appendix-b-build-process-overview.mdx)
