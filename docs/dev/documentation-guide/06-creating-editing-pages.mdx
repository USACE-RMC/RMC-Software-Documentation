...
<TableVerticalNoRef
  alt="Available front matter fields"
  headers={[[{ value: 'Field' }, { value: 'Type' }, { value: 'Description' }, { value: 'Example' }]]}
  columns={[
    [
      '<code>title</code>',
      '<code>sidebar_label</code>',
    ...
    ['String', 'String', 'String', 'String', 'String', 'String', 'Array', 'String', 'Array', 'String', 'String', 'String', 'String', 'Boolean'],
    [
      'Page title (required)',
      'Override sidebar text',
      'Document date',
      'Document type',
      'Full document title',
      'Subtitle',
      ...
      'Acknowledgment text',
      'Mark page as draft (displays watermark)',
    ],
    [
      '<code>"Methodology"</code>',
      '<code>"Method"</code>',
      '<code>"November 2023"</code>',
      '<code>"User\'s Guide"</code>',
      '<code>"LifeSim User\'s Guide"</code>',
      '<code>"Version 1.0"</code>',
      '<code>["Author Name"]</code>',
      '<code>"This guide..."</code>',
      '<code>["topic1", "topic2"]</code>',
      '<code>"Tim O\'Leary"</code>',
      '<code>"502-315-6599"</code>',
      '<code>"O\'Leary, T. (2023)..."</code>',
      '<code>"The authors thank..."</code>',
      '<code>true</code>',
    ],
  ]}
/>
...
For numbered video embeds with captions, use the `Video` component:
...
To cross-reference a video from text, use `VideoReference`:
...
The site supports interactive live code blocks where readers can edit and run JSX code directly in the browser. Add `live` after the language identifier:

````md
```jsx live
function Demo() {
  return <p>Edit me and see the result!</p>;
}
```
````

:::tip
Live code blocks are powered by `@docusaurus/theme-live-codeblock`. They are most useful for interactive examples but are rarely needed in RMC documentation.
:::
...
<TableVerticalNoRef
  alt="LaTeX symbol reference"
  headers={[[{ value: 'Symbol' }, { value: 'LaTeX' }, { value: 'Symbol' }, { value: 'LaTeX' }]]}
  columns={[
    ['α', 'γ', '∑', '√', '≤', '±'],
    ['<code>\\alpha</code>', '<code>\\gamma</code>', '<code>\\sum</code>', '<code>\\sqrt{x}</code>', '<code>\\leq</code>', '<code>\\pm</code>'],
    ['β', 'Δ', '∫', '∞', '≥', '×'],
    ['<code>\\beta</code>', '<code>\\Delta</code>', '<code>\\int</code>', '<code>\\infty</code>', '<code>\\geq</code>', '<code>\\times</code>'],
  ]}
/>
...
```jsx
<Figure figKey="figure-1" src="/figures/path/to/image.png" alt="Description" caption="Figure caption text" />
```
...
- Figures: `figKey="figure-3"`
- Tables: `tableKey="table-1"`
- Equations: `equationKey="equation-1"`
...
```jsx
See <FigureReference figKey="figure-3" /> for the analysis workflow.
```

#### Link to Tables

```jsx
<TableReference tableKey="table-1" /> shows the final results.
```

#### Link to Equations

```jsx
Using <EquationReference equationKey="equation-1" />, we can solve...
```
...
<Figure figKey="figure-3" src="/figures/workflow.png" alt="Flowchart showing analysis steps" caption="Analysis workflow diagram" />

## Output Visualization

<Figure figKey="figure-5" src="/figures/output.png" alt="Graph showing results over time" caption="Model output results" />

## Discussion

The analysis follows the workflow shown in <FigureReference figKey="figure-3" />. Results are visualized in <FigureReference figKey="figure-5" />, demonstrating clear trends over the simulation period.
```
...
Risk is calculated as the product of probability and consequence, as shown in <EquationReference equationKey="equation-2" />:
...
For multiple events, total risk is the sum of individual risks as defined in <EquationReference equationKey="equation-3" />:
...
- Stop the dev server (Ctrl+C) and restart it with `npm start` - numbering scripts run automatically on startup
...
- Stop the dev server (Ctrl+C) and restart it with `npm start` - sidebar generation runs automatically on startup
