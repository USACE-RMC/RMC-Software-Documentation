---
title: React Components Reference
sidebar_label: React Components
---

import NavContainer from '@site/src/components/NavContainer';
import Button from '@site/src/components/Button';
import TableVerticalNoRef from '@site/src/components/TableVerticalNoRef';

<NavContainer link="/dev" linkTitle="Developer Resources" />

# React Components Reference

## Introduction

This comprehensive guide documents all React components available for use in your MDX documentation. Components are organized by category to help you quickly find the right tool for your needs.

### What Are React Components?

**React components** are reusable UI elements that you can insert into your MDX files. They handle complex layouts, automatic numbering, and consistent styling without requiring you to write HTML or CSS.

**Benefits:**

- ✅ Automatic numbering (figures, tables, equations, videos)
- ✅ Cross-referencing between elements
- ✅ Consistent formatting across all documentation
- ✅ Professional appearance
- ✅ Responsive design (works on all screen sizes)

### How to Use Components

**Basic workflow:**

1. **Import** the component at the top of your MDX file
2. **Use** the component with JSX syntax
3. **Configure** with props (properties)

**Example:**

```jsx
---
title: My Chapter
---

import Figure from '@site/src/components/Figure';

# My Chapter

<Figure
  figKey="figure-3"
  src="/figures/my-document/workflow.png"
  alt="Workflow diagram"
  caption="Overall analysis workflow"
  width="80%"
/>
```

---

## Component Categories

Components are organized into these categories:

- **[Document Metadata](#document-metadata)** - Document info and version history
- **[Figures & Images](#figures--images)** - Numbered figures, inline images, references
- **[Tables](#tables)** - Various table layouts and references
- **[Equations](#equations)** - LaTeX equations with automatic numbering
- **[Citations](#citations)** - Bibliography and citation management
- **[Content Organization](#content-organization)** - Tabs, lists, processes
- **[Multimedia](#multimedia)** - Videos with captions
- **[Navigation](#navigation)** - Buttons, links, containers

---

## Quick Component Selector

**Need to display an image?**

- Numbered figure with caption → [`<Figure>`](#figure-)
- Unnumbered inline image → [`<FigureInline>`](#figureinline)
- Simple image, no caption → [`<FigureNoRef>`](#figurenoref)

**Need to create a table?**

- Standard columnar table (header cells across the top) → [`<TableVertical>`](#tablevertical-)
- Row-based table (header cells on the left) → [`<TableHorizontal>`](#tablehorizontal)
- Two-column table to document acronyms → [`<TableAcronyms>`](#tableacronyms)

**Need to show a process?**

- Step-by-step procedure → [`<ProcessList>`](#processlist-)
- Expandable grouped items → [`<CollectionList>`](#collectionlist-)
- Tabbed alternatives → [`<DocTabs>`](#doctabs-)

**Need to add equations?**

- Numbered equation → [`<Equation>`](#equation-)
- Unnumbered equation → [`<EquationNoRef>`](#equationnoref)
- Inline equation badge → [`<EquationChip>`](#equationchip)

---

## Document Metadata

### DocumentMetadata ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Displays report metadata in a professional table format at the beginning of documents.

**When to use:**

- `00-document-info.mdx` files
- Beginning of technical reports
- Any document requiring formal metadata

**Import:**

```jsx
import DocumentMetadata from '@site/src/components/DocumentMetadata';
```

**Usage:**

```jsx
---
reportDate: November 2023
reportType: User's Guide
reportTitle: LifeSim Life Safety Model
reportSubTitle: Version 1.0
reportAuthors: ["John Doe, Risk Management Center", "Jane Smith, RMC"]
reportAbstract: This guide provides instructions for using LifeSim software...
reportSubjectTerms: ["life safety", "dam failure", "risk analysis"]
responsiblePersonName: John Doe
responsiblePersonNumber: 555-0100
---

import DocumentMetadata from '@site/src/components/DocumentMetadata';

<DocumentMetadata metadata={frontMatter} />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>metadata</code>'],
    ['Object'],
    ['Yes'],
    ['Front matter object — pass <code>{`{frontMatter}`}</code>'],
  ]}
/>

**Front Matter Fields:**

<TableVerticalNoRef
  alt="Table: Field, Type, Description"
  headers={[[{ value: 'Field' }, { value: 'Type' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>reportDate</code>',
      '<code>reportType</code>',
      '<code>reportTitle</code>',
      '<code>reportSubTitle</code>',
      '<code>reportAuthors</code>',
      '<code>reportAbstract</code>',
      '<code>reportSubjectTerms</code>',
      '<code>responsiblePersonName</code>',
      '<code>responsiblePersonNumber</code>',
      '<code>citationGuide</code>',
      '<code>reportAcknowledgments</code>',
    ],
    ['String', 'String', 'String', 'String', 'Array', 'String', 'Array', 'String', 'String', 'String', 'String'],
    [
      'Publication date',
      'Document type (User&#x27;s Guide, Technical Manual, etc.)',
      'Full document title',
      'Subtitle or version',
      'List of author names',
      'Document summary',
      'Keywords/topics',
      'Point of contact',
      'Contact phone number',
      'How to cite this document',
      'Acknowledgment text',
    ],
  ]}
/>

---

### TableVersionHistory ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Displays document version history in a formatted table.

**When to use:**

- `00-version-history.mdx` files
- Tracking document revisions

**Import:**

```jsx
import TableVersionHistory from '@site/src/components/TableVersionHistory';
```

**Usage:**

```jsx
<TableVersionHistory
  versions={['1.0', '1.1']}
  dates={['November 2023', 'March 2024']}
  descriptions={['Initial release', 'Updated methodology section, added new examples']}
  modifiedBy={['J. Doe', 'J. Doe']}
  reviewedBy={['J. Smith', 'J. Smith']}
  approvedBy={['A. Manager', 'A. Manager']}
/>
```

**Advanced Usage with Multiple Description Items:**

```jsx
<TableVersionHistory
  versions={['1.0', '1.1']}
  dates={['November 2023', 'March 2024']}
  descriptions={['Initial release', ['Updated methodology section', 'Added new examples', 'Fixed typos']]}
  modifiedBy={['J. Doe', 'J. Doe']}
  reviewedBy={['J. Smith', 'J. Smith']}
  approvedBy={['A. Manager', 'A. Manager']}
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>versions</code>',
      '<code>dates</code>',
      '<code>descriptions</code>',
      '<code>modifiedBy</code>',
      '<code>reviewedBy</code>',
      '<code>approvedBy</code>',
    ],
    ['Array', 'Array', 'Array', 'Array', 'Array', 'Array'],
    ['No', 'No', 'No', 'No', 'No', 'No'],
    ['<code>[]</code>', '<code>[]</code>', '<code>[]</code>', '<code>[]</code>', '<code>[]</code>', '<code>[]</code>'],
    [
      'Version numbers (e.g., &quot;1.0&quot;)',
      'Release dates',
      'Description of changes (string or array of strings for bullet list)',
      'Document authors',
      'Technical reviewers',
      'Approving authorities',
    ],
  ]}
/>

---

## Figures & Images

### Figure ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Numbered figure with caption and automatic cross-referencing.

**When to use:**

- Any diagram, chart, or image that needs a figure number
- Images that will be referenced from text
- Professional documentation figures

**Import:**

```jsx
import Figure from '@site/src/components/Figure';
```

**Basic Usage:**

```jsx
<Figure
  figKey="figure-3"
  src="/figures/desktop-applications/lifesim/users-guide/v1.0/workflow.png"
  alt="Flowchart showing three-step analysis process"
  caption="LifeSim analysis workflow"
/>
```

**Advanced Usage:**

```jsx
<Figure
  figKey="figure-6"
  src="/figures/my-software/results-graph.png"
  alt="Bar chart comparing model results across three scenarios"
  caption="Comparison of model results for baseline, alternative A, and alternative B scenarios"
  width="90%"
  background="transparent"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>figKey</code>',
      '<code>src</code>',
      '<code>alt</code>',
      '<code>caption</code>',
      '<code>width</code>',
      '<code>background</code>',
      '<code>id</code>',
    ],
    ['String', 'String', 'String', 'String', 'String', 'String', 'String'],
    ['Yes', 'Yes', 'Yes', 'Yes', 'No', 'No', 'No'],
    ['-', '-', '-', '-', '<code>&quot;80%&quot;</code>', '<code>&quot;filled&quot;</code>', '-'],
    [
      'Unique identifier for numbering and cross-refs',
      'Image path (relative to static folder)',
      'Accessibility description',
      'Figure caption text',
      'Display width',
      '<code>&quot;filled&quot;</code> or <code>&quot;transparent&quot;</code> background',
      'Custom HTML id (defaults to figKey)',
    ],
  ]}
/>

**Tips:**

- Use sequential `figKey` values: `figure-1`, `figure-2`, `figure-3`
- Always start paths with `/figures/`
- Write detailed `alt` text for accessibility
- Keep captions concise but descriptive

**Common Mistakes:**

- ❌ Forgetting leading slash: `figures/image.png`
  - ✅ Correct: `/figures/image.png`
- ❌ Duplicate `figKey` values
  - ✅ Each figure needs unique key

---

### FigureReference ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Creates cross-references to numbered figures.

**When to use:**

- Referencing figures from body text
- Creating figure call-outs

**Import:**

```jsx
import FigureReference from '@site/src/components/FigureReference';
```

**Usage:**

```jsx
The analysis workflow is shown in <FigureReference figKey="figure-3" />.

See <FigureReference figKey="figure-6" /> for detailed results.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>figKey</code>', '<code>suffix</code>'],
    ['String', 'String'],
    ['Yes', 'No'],
    ['Must match a Figure&#x27;s figKey', 'Text appended after the number (e.g., <code>"a"</code> renders "Figure 1a")'],
  ]}
/>

**Output:** Renders as clickable link: "Figure 1", "Figure 2", etc.

---

### FigureInline

**Purpose:** Small inline image that displays at text height (1em), useful for icons or inline graphics.

**When to use:**

- Inline icons within text
- Small decorative graphics
- Inline symbols or badges

**Import:**

```jsx
import FigureInline from '@site/src/components/FigureInline';
```

**Usage:**

```jsx
Click the <FigureInline src="/figures/icons/settings-icon.png" /> icon to open settings.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>src</code>'], ['String'], ['Yes'], ['Image path (relative to static folder)']]}
/>

---

### FigureNoRef

**Purpose:** Simple image display without caption or numbering.

**When to use:**

- Quick image insertion
- Screenshots in tutorials
- Images that don't need captions

**Import:**

```jsx
import FigureNoRef from '@site/src/components/FigureNoRef';
```

**Usage:**

```jsx
<FigureNoRef src="/figures/icons/warning-icon.png" alt="Warning icon" width="100px" />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>src</code>', '<code>alt</code>', '<code>width</code>', '<code>background</code>'],
    ['String', 'String', 'String', 'String'],
    ['Yes', 'Yes', 'No', 'No'],
    ['-', '-', '<code>&quot;80%&quot;</code>', '<code>&quot;filled&quot;</code>'],
    ['Image path', 'Accessibility description', 'Display width', '<code>&quot;filled&quot;</code> or <code>&quot;transparent&quot;</code>'],
  ]}
/>

---

## Tables

### TableVertical ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Standard columnar table with automatic numbering.

**When to use:**

- Standard data tables
- Comparison tables
- Results tables
- Most tabular data

**Import:**

```jsx
import TableVertical from '@site/src/components/TableVertical';
```

**Basic Usage:**

```jsx
<TableVertical
  tableKey="table-3"
  alt="Model input parameters"
  headers={[[{ value: 'Parameter' }, { value: 'Value' }, { value: 'Units' }]]}
  columns={[
    ['Population', 'Flow Rate', 'Duration'],
    ['10,000', '5,000', '24'],
    ['people', 'cfs', 'hours'],
  ]}
  caption="Model input parameters"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>tableKey</code>',
      '<code>alt</code>',
      '<code>headers</code>',
      '<code>columns</code>',
      '<code>caption</code>',
      '<code>colWidths</code>',
      '<code>colAlign</code>',
      '<code>headerAlign</code>',
      '<code>colVAlign</code>',
      '<code>headerVAlign</code>',
      '<code>widthMode</code>',
      '<code>footnotes</code>',
      '<code>id</code>',
    ],
    ['String', 'String', 'Array', 'Array', 'String', 'Array', 'Array', 'Array', 'Array', 'Array', 'String', 'Array', 'String'],
    ['Yes', 'Yes', 'Yes', 'Yes', 'Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '<code>&quot;full&quot;</code>', '-', '-'],
    [
      'Unique identifier',
      'Accessibility description',
      'Array of header row arrays',
      'Array of column arrays',
      'Table caption',
      'Column widths (e.g., <code>[14, &quot;20ch&quot;, &quot;minmax(16ch, 1fr)&quot;]</code>)',
      'Column horizontal alignment (e.g., <code>[&quot;left&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Header horizontal alignment (e.g., <code>[&quot;center&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Column vertical alignment (e.g., <code>[&quot;top&quot;, &quot;middle&quot;, &quot;bottom&quot;]</code>)',
      'Header vertical alignment (e.g., <code>[&quot;middle&quot;, &quot;bottom&quot;, &quot;middle&quot;]</code>)',
      '<code>&quot;full&quot;</code> or <code>&quot;intrinsic&quot;</code>',
      'Array of footnote strings or ReactNodes',
      'Custom HTML id (defaults to tableKey)',
    ],
  ]}
/>

**Advanced Usage with Merged Cells:**

```jsx
<TableVertical
  tableKey="table-4"
  alt="Example table with merged cells"
  headers={[
    [
      { value: 'Scenario' },
      { value: 'Hydraulic Parameters', colSpan: 2 }, // This header spans 2 columns
      { value: 'Notes' },
    ],
  ]}
  columns={[
    // Column 1: Scenario names
    [
      { value: 'Baseline', rowSpan: 2 }, // This cell spans 2 rows, so row 2 in this column is AUTO-SKIPPED
      'Alternative A',
      'Alternative B',
    ],
    // Column 2: Flow rates
    [
      '5,000 cfs', // Row 1
      '5,000 cfs', // Row 2 (Baseline's 2nd row)
      '7,500 cfs', // Row 3 (Alternative A)
      '10,000 cfs', // Row 4 (Alternative B)
    ],
    // Column 3: Velocities
    [
      '10 fps', // Row 1
      '12 fps', // Row 2 (Baseline's 2nd row)
      '15 fps', // Row 3 (Alternative A)
      '18 fps', // Row 4 (Alternative B)
    ],
    // Column 4: Notes
    [
      'Normal conditions', // Row 1
      'High water', // Row 2 (Baseline's 2nd row)
      'Moderate risk', // Row 3 (Alternative A)
      'High risk scenario', // Row 4 (Alternative B)
    ],
  ]}
  caption="Table demonstrating merged cells"
/>
```

**Important Notes on Merged Cells:**

- **Column spanning (`colSpan`)**: Used in `headers` to make a header span multiple columns. Other header cells in that row are pushed to the right.

- **Row spanning (`rowSpan`)**: When a cell spans multiple rows, you MUST still provide values for ALL rows in the other columns. The component handles skipping the spanned cells automatically.

- **Key rule**: Think of `columns` as providing data for every visible row. If Column 1 Row 1 has `rowSpan: 2`, you still need to provide data for Column 2 Row 2, Column 3 Row 2, etc. The `rowSpan` only affects its own column.

- **Counting rows**: In the example above, "Baseline" spans 2 rows, creating 4 total rows (2 for Baseline + 1 for Alt A + 1 for Alt B). All other columns must have 4 values.

**Advanced Usage with Alignment and Footnotes:**

```jsx
<TableVertical
  tableKey="table-5"
  alt="Advanced table with custom alignment"
  headers={[[{ value: 'Item' }, { value: 'Value' }, { value: 'Units' }]]}
  columns={[
    ['Population', 'Flow Rate', 'Duration'],
    ['10,000', '5,000', '24'],
    ['people', 'cfs', 'hours'],
  ]}
  caption="Model parameters with footnotes"
  colAlign={['left', 'right', 'center']}
  headerAlign={['center', 'center', 'center']}
  colWidths={['40%', '30%', '30%']}
  footnotes={['Flow rate measured at dam outlet', 'Duration represents time from breach initiation']}
/>
```

---

### TableVerticalNoRef

**Purpose:** Standard columnar table **without** automatic numbering or cross-referencing.

**When to use:**

- Documentation tables (like props tables, reference tables)
- Tables that don't need figure numbers
- Tables that won't be referenced from text
- Quick reference tables and guides

**Import:**

```jsx
import TableVerticalNoRef from '@site/src/components/TableVerticalNoRef';
```

**Usage:**

```jsx
<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>alt</code>',
      '<code>headers</code>',
      '<code>columns</code>',
      '<code>colWidths</code>',
      '<code>colAlign</code>',
      '<code>headerAlign</code>',
      '<code>colVAlign</code>',
      '<code>headerVAlign</code>',
      '<code>widthMode</code>',
      '<code>footnotes</code>',
    ],
    ['String', 'Array', 'Array', 'Array', 'Array', 'Array', 'Array', 'Array', 'String', 'Array'],
    ['Yes', 'Yes', 'Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    ['-', '-', '-', '-', '-', '-', '-', '-', '<code>&quot;full&quot;</code>', '-'],
    [
      'Accessibility description',
      'Array of header row arrays',
      'Array of column arrays',
      'Column widths (e.g., <code>[14, &quot;20ch&quot;, &quot;minmax(16ch, 1fr)&quot;]</code>)',
      'Column horizontal alignment (e.g., <code>[&quot;left&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Header horizontal alignment (e.g., <code>[&quot;center&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Column vertical alignment (e.g., <code>[&quot;top&quot;, &quot;middle&quot;, &quot;bottom&quot;]</code>)',
      'Header vertical alignment (e.g., <code>[&quot;middle&quot;, &quot;bottom&quot;, &quot;middle&quot;]</code>)',
      '<code>&quot;full&quot;</code> or <code>&quot;intrinsic&quot;</code>',
      'Array of footnote strings or ReactNodes',
    ],
  ]}
/>
```

**Renders as:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>alt</code>',
      '<code>headers</code>',
      '<code>columns</code>',
      '<code>colWidths</code>',
      '<code>colAlign</code>',
      '<code>headerAlign</code>',
      '<code>colVAlign</code>',
      '<code>headerVAlign</code>',
      '<code>widthMode</code>',
      '<code>footnotes</code>',
    ],
    ['String', 'Array', 'Array', 'Array', 'Array', 'Array', 'Array', 'Array', 'String', 'Array'],
    ['Yes', 'Yes', 'Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    ['-', '-', '-', '-', '-', '-', '-', '-', '<code>&quot;full&quot;</code>', '-'],
    [
      'Accessibility description',
      'Array of header row arrays',
      'Array of column arrays',
      'Column widths (e.g., <code>[14, &quot;20ch&quot;, &quot;minmax(16ch, 1fr)&quot;]</code>)',
      'Column horizontal alignment (e.g., <code>[&quot;left&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Header horizontal alignment (e.g., <code>[&quot;center&quot;, &quot;center&quot;, &quot;right&quot;]</code>)',
      'Column vertical alignment (e.g., <code>[&quot;top&quot;, &quot;middle&quot;, &quot;bottom&quot;]</code>)',
      'Header vertical alignment (e.g., <code>[&quot;middle&quot;, &quot;bottom&quot;, &quot;middle&quot;]</code>)',
      '<code>&quot;full&quot;</code> or <code>&quot;intrinsic&quot;</code>',
      'Array of footnote strings or ReactNodes',
    ],
  ]}
/>

**Key Differences from TableVertical:**

- ❌ No `tableKey` prop (not numbered)
- ❌ No `caption` prop (no caption display)
- ❌ No `id` prop (no anchor linking)
- ❌ Cannot be referenced with `<TableReference>`
- ✅ All other features identical (merged cells, alignment, footnotes, etc.)

**When to Choose:**

- Use `TableVertical` for content tables that need numbering ("Table 1", "Table 2")
- Use `TableVerticalNoRef` for meta-tables (props, references, documentation tables)

---

### TableReference ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Cross-references to numbered tables.

**When to use:**

- Referencing tables from text

**Import:**

```jsx
import TableReference from '@site/src/components/TableReference';
```

**Usage:**

```jsx
Input parameters are listed in <TableReference tableKey="table-3" />.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>tableKey</code>'], ['String'], ['Yes'], ['Must match a table&#x27;s tableKey']]}
/>

---

### TableHorizontal

**Purpose:** Row-based table where each row represents a record.

**When to use:**

- Entity descriptions
- Feature comparisons
- Record-based data

**Import:**

```jsx
import TableHorizontal from '@site/src/components/TableHorizontal';
```

**Usage:**

```jsx
<TableHorizontal
  tableKey="table-6"
  alt="Software feature comparison"
  headers={['Platform', 'Open Source', 'Runtime']}
  rows={[
    ['Windows', 'Yes', '< 1 hour'],
    ['Windows, Mac', 'No', '2-3 hours'],
  ]}
  caption="Comparison of software features"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>tableKey</code>', '<code>alt</code>', '<code>headers</code>', '<code>rows</code>', '<code>caption</code>', '<code>colWidths</code>', '<code>colAlign</code>', '<code>headerAlign</code>', '<code>widthMode</code>', '<code>footnotes</code>'],
    ['String', 'String', 'Array', 'Array', 'String/Node', 'Array', 'Array', 'Array', 'String', 'Array'],
    ['Yes', 'Yes', 'Yes', 'Yes', 'Yes', 'No', 'No', 'No', 'No', 'No'],
    ['Unique identifier', 'Accessibility description', 'Array of row header labels (first column)', 'Array of arrays — each inner array is one row of data cells', 'Table caption', 'Column widths (e.g., <code>["30%", "70%"]</code>)', 'Column text alignment', 'Header text alignment', '<code>"full"</code> (default) or <code>"auto"</code>', 'Array of footnote strings'],
  ]}
/>

---

### TableAcronyms

**Purpose:** Two-column table specifically for acronyms and abbreviations.

**When to use:**

- Appendix for acronyms
- Abbreviation lists

**Import:**

```jsx
import TableAcronyms from '@site/src/components/TableAcronyms';
```

**Usage:**

```jsx
<TableAcronyms
  headers={['Acronym', 'Full Form']}
  columns={[
    ['USACE', 'RMC', 'DST'],
    ['U.S. Army Corps of Engineers', 'Risk Management Center', 'Dam Safety Toolbox'],
  ]}
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>headers</code>', '<code>columns</code>'],
    ['Array', 'Array'],
    ['Yes', 'Yes'],
    ['Column header labels (e.g., <code>["Acronym", "Full Form"]</code>)', 'Array of arrays — each inner array is one column of values'],
  ]}
/>

---

## Equations

### Equation ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Numbered LaTeX equation with automatic numbering.

**When to use:**

- Mathematical formulas that need numbering
- Equations referenced from text

**Import:**

```jsx
import Equation from '@site/src/components/Equation';
```

**Basic Usage:**

```jsx
<Equation equationKey="equation-2" equation="R = P \times C" />
```

**Advanced Usage:**

```jsx
<Equation equationKey="equation-1" equation="\frac{-b \pm \sqrt{b^2 - 4ac}}{2a}" />
```

**Complex Example:**

```jsx
<Equation equationKey="equation-3" equation="R_{total} = \sum_{i=1}^{n} P_i \times C_i" />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>equationKey</code>', '<code>equation</code>', '<code>inline</code>', '<code>id</code>'],
    ['String', 'String', 'Boolean', 'String'],
    ['Yes', 'Yes', 'No', 'No'],
    ['Unique identifier', 'LaTeX equation string', 'Render inline instead of block (default: <code>false</code>)', 'Custom HTML id attribute'],
  ]}
/>

**LaTeX Quick Reference:**

**Common Operations:**

<TableVerticalNoRef
  alt="Table: Operation, LaTeX, Example"
  headers={[[{ value: 'Operation' }, { value: 'LaTeX' }, { value: 'Example' }]]}
  columns={[
    ['Fraction', 'Square root', 'Subscript', 'Superscript', 'Both sub/super'],
    [
      '<code>\\frac{numerator}{denominator}</code>',
      '<code>\\sqrt{x}</code> or <code>\\sqrt[n]{x}</code>',
      '<code>x_i</code> or <code>x_{ij}</code>',
      '<code>x^2</code> or <code>x^{n+1}</code>',
      '<code>x_i^2</code>',
    ],
    ['a/b', '√x or ⁿ√x', 'xᵢ or xᵢⱼ', 'x² or xⁿ⁺¹', 'xᵢ²'],
  ]}
/>

**Summation, Products, and Integrals:**

<TableVerticalNoRef
  alt="Table: Symbol, LaTeX, Example"
  headers={[[{ value: 'Symbol' }, { value: 'LaTeX' }, { value: 'Example' }]]}
  columns={[
    ['Summation', 'Product', 'Integral', 'Double integral', 'Limit'],
    [
      '<code>\\sum_{i=1}^{n}</code>',
      '<code>\\prod_{i=1}^{n}</code>',
      '<code>\\int_a^b</code>',
      '<code>\\iint_D</code>',
      '<code>\\lim_{x \\to \\infty}</code>',
    ],
    ['∑ᵢ₌₁ⁿ', '∏ᵢ₌₁ⁿ', '∫ₐᵇ', '∬ᴰ', 'lim x→∞'],
  ]}
/>

**Greek Letters (Common):**

<TableVerticalNoRef
  alt="Table: Letter, LaTeX, Letter, LaTeX"
  headers={[[{ value: 'Letter' }, { value: 'LaTeX' }, { value: 'Letter' }, { value: 'LaTeX' }]]}
  columns={[
    ['α (alpha)', 'β (beta)', 'γ (gamma)', 'Γ (Gamma)', 'δ (delta)', 'Δ (Delta)', 'ε (epsilon)', 'θ (theta)'],
    [
      '<code>\\alpha</code>',
      '<code>\\beta</code>',
      '<code>\\gamma</code>',
      '<code>\\Gamma</code>',
      '<code>\\delta</code>',
      '<code>\\Delta</code>',
      '<code>\\epsilon</code>',
      '<code>\\theta</code>',
    ],
    ['λ (lambda)', 'Λ (Lambda)', 'μ (mu)', 'π (pi)', 'Π (Pi)', 'σ (sigma)', 'Σ (Sigma)', 'φ (phi)'],
    [
      '<code>\\lambda</code>',
      '<code>\\Lambda</code>',
      '<code>\\mu</code>',
      '<code>\\pi</code>',
      '<code>\\Pi</code>',
      '<code>\\sigma</code>',
      '<code>\\Sigma</code>',
      '<code>\\phi</code>',
    ],
  ]}
/>

**Comparison and Logic:**

<TableVerticalNoRef
  alt="Table: Symbol, LaTeX, Symbol, LaTeX"
  headers={[[{ value: 'Symbol' }, { value: 'LaTeX' }, { value: 'Symbol' }, { value: 'LaTeX' }]]}
  columns={[
    ['≤', '≥', '≠', '≈', '∝'],
    [
      '<code>\\leq</code> or <code>\\le</code>',
      '<code>\\geq</code> or <code>\\ge</code>',
      '<code>\\neq</code> or <code>\\ne</code>',
      '<code>\\approx</code>',
      '<code>\\propto</code>',
    ],
    ['≡', '∈', '∉', '⊂', '⊆'],
    ['<code>\\equiv</code>', '<code>\\in</code>', '<code>\\notin</code>', '<code>\\subset</code>', '<code>\\subseteq</code>'],
  ]}
/>

**Operations and Symbols:**

<TableVerticalNoRef
  alt="Table: Symbol, LaTeX, Symbol, LaTeX"
  headers={[[{ value: 'Symbol' }, { value: 'LaTeX' }, { value: 'Symbol' }, { value: 'LaTeX' }]]}
  columns={[
    ['×', '÷', '±', '∓', '·'],
    ['<code>\\times</code>', '<code>\\div</code>', '<code>\\pm</code>', '<code>\\mp</code>', '<code>\\cdot</code>'],
    ['∞', '∂', '∇', '∀', '∃'],
    ['<code>\\infty</code>', '<code>\\partial</code>', '<code>\\nabla</code>', '<code>\\forall</code>', '<code>\\exists</code>'],
  ]}
/>

**Parentheses and Brackets:**

<TableVerticalNoRef
  alt="Table: Type, LaTeX, Example"
  headers={[[{ value: 'Type' }, { value: 'LaTeX' }, { value: 'Notes' }]]}
  columns={[
    ['Parentheses', 'Brackets', 'Braces', 'Auto-sizing', 'Angle brackets'],
    ['<code>( )</code>', '<code>[ ]</code>', '<code>\\{ \\}</code>', '<code>\\left( \\frac{a}{b} \\right)</code>', '<code>\\langle \\rangle</code>'],
    ['Regular parentheses', 'Regular brackets', 'Braces require backslash', 'Auto-size to content height', 'For inner products, averages'],
  ]}
/>

**Examples:**

<TableVerticalNoRef
  alt="Table: Description, LaTeX"
  headers={[[{ value: 'Description' }, { value: 'LaTeX Code' }]]}
  columns={[
    ['Quadratic formula', 'Binomial coefficient', 'Partial derivative', 'Matrix/vector notation', 'Probability'],
    [
      '<code>x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}</code>',
      '<code>\\binom{n}{k} = \\frac{n!}{k!(n-k)!}</code>',
      '<code>\\frac{\\partial f}{\\partial x}</code>',
      '<code>\\mathbf{x} = [x_1, x_2, \\ldots, x_n]^T</code>',
      '<code>P(A|B) = \\frac{P(B|A)P(A)}{P(B)}</code>',
    ],
  ]}
/>

---

### EquationReference

**Purpose:** Cross-references to numbered equations.

**When to use:**

- Referencing equations from text

**Import:**

```jsx
import EquationReference from '@site/src/components/EquationReference';
```

**Usage:**

```jsx
Using <EquationReference equationKey="equation-2" />, we calculate total risk.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>equationKey</code>'], ['String'], ['Yes'], ['Must match an Equation&#x27;s equationKey']]}
/>

---

### EquationNoRef

**Purpose:** Unnumbered LaTeX equation (display only).

**When to use:**

- Supporting equations without numbering
- Equation examples

**Import:**

```jsx
import EquationNoRef from '@site/src/components/EquationNoRef';
```

**Usage:**

```jsx
<EquationNoRef equation="E = mc^2" />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>equation</code>', '<code>inline</code>'],
    ['String', 'Boolean'],
    ['Yes', 'No'],
    ['LaTeX equation string', 'Render inline instead of block (default: <code>true</code>)'],
  ]}
/>

---

### EquationChip

**Purpose:** Inline badge/chip for short equations or variable names.

**When to use:**

- Inline variable references
- Short formulas in text
- Mathematical notation badges

**Import:**

```jsx
import EquationChip from '@site/src/components/EquationChip';
```

**Usage:**

```jsx
The variable <EquationChip>$P$</EquationChip> represents probability, while <EquationChip>$C$</EquationChip> represents consequence.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>children</code>'], ['ReactNode'], ['Yes'], ['Content to display (typically LaTeX in $...$)']]}
/>

---

## Citations

**Citation Numbering:** Citations are numbered based on the **order they first appear in the document**, not alphabetically. The first citation you use becomes [1], the second becomes [2], and so on.

### Citation

**Purpose:** Inline citation reference that displays a numbered bracket link.

**When to use:**

- Citing sources in text
- Referencing bibliography entries

**Import:**

```jsx
import Citation from '@site/src/components/Citation';
```

**Usage:**

```jsx
The methodology was developed by Smith <Citation citationKey="Smith2020" />.

Multiple authors have studied this topic <Citation citationKey="Jones2018" /> <Citation citationKey="Brown2019" />.
```

:::warning IMPORTANT CITATION RULES

⚠️ **Always include author/year text before the `<Citation>` component.** The citation number should never appear alone without context.

```jsx
✅ Good - Author/year precedes citation:
The methodology was developed by Smith <Citation citationKey="Smith2020" />.
Recent studies (Jones 2018 <Citation citationKey="Jones2018" />) show similar results.

❌ Bad - Citation number hanging alone:
The methodology was developed by <Citation citationKey="Smith2020" />.
<Citation citationKey="Jones2018" /> shows similar results.
```

:::

**Best Practices:**

- Include author name and/or year before the Citation component
- Use natural sentence structure: "Smith (2020) demonstrated..." or "...was demonstrated by Smith [citation]."
- For multiple citations, provide context for each: "Several studies (Jones 2018 [1], Brown 2019 [2]) have shown..."

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>citationKey</code>'], ['String'], ['Yes'], ['Must match key in bib.json']]}
/>

**Output:** Renders as: [1], [2], etc. (clickable link to the citation in the CitationFootnote)

:::info Numbering:
Citations are numbered in the order they first appear in the document.
:::

---

### CitationFootnote ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Displays list of citations used on current page in order of appearance.

**When to use:**

- End of each page with citations
- Before full bibliography

**Import:**

```jsx
import CitationFootnote from '@site/src/components/CitationFootnote';
```

**Usage:**

```jsx
## References

<CitationFootnote />
```

**Props:** None

:::warning Missing CitationFootnote
Insert the `<CitationFootnote>` component at the end of each MDX page. Failure to do so could result in
build errors.
:::

---

### Bibliography

**Purpose:** Full bibliography listing all references for a document in order of appearance.

**When to use:**

- References appendix (typically at the end of a document)

**Import:**

```jsx
import Bibliography from '@site/src/components/Bibliography';
```

**Usage:**

```jsx
## Appendix: Bibliography

<Bibliography />
```

**Props:** None

**How It Works:**

The Bibliography component automatically determines which `bib.json` file to load based on the current page's location
in the file structure. It maps the document path in `/docs/` to the corresponding `bib.json` file in `/bibliographies/`,
then displays all citations used throughout the document in numerical order based on first appearance (not alphabetically).

:::warning `bib.json` Placement
Ensure your `bib.json` file is placed in the correct location within the bibliographies directory structure. Incorrect
placement will produce an error from the `<Bibliography>` component.
:::

---

## Content Organization

### ProcessList ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Step-by-step numbered procedures with optional sub-steps, expandable details, and freeform content blocks.

**When to use:**

- Installation instructions
- Workflows and procedures
- Multi-step tutorials
- Configuration guides
- Any sequential process requiring clear numbering

**Import:**

```jsx
import ProcessList from '@site/src/components/ProcessList';
```

#### Example 1: Simple Numbered List

Basic numbered steps without sub-steps or details:

```jsx
<ProcessList
  items={[
    { title: 'Install Python 3.8 or later' },
    { title: 'Download the software package from the website' },
    { title: 'Run the installer and follow the prompts' },
    { title: 'Verify installation by running python --version' },
  ]}
/>
```

**Renders as:**

import ProcessList from '@site/src/components/ProcessList';

<ProcessList
  items={[
    { title: 'Install Python 3.8 or later' },
    { title: 'Download the software package from the website' },
    { title: 'Run the installer and follow the prompts' },
    { title: 'Verify installation by running python --version' },
  ]}
/>

---

#### Example 2: Multi-Level Steps (Nested Sub-steps)

Steps with lettered sub-steps (a, b, c, etc.):

```jsx
<ProcessList
  items={[
    {
      title: 'Prepare your development environment',
      child: [
        { title: 'Install Node.js version 18 or later' },
        { title: 'Install Visual Studio Code or your preferred editor' },
        { title: 'Install Git for version control' },
      ],
    },
    {
      title: 'Clone and configure the project',
      child: [
        { title: 'Clone the repository from GitHub' },
        { title: 'Navigate to the project directory' },
        { title: 'Copy .env.example to .env and configure environment variables' },
      ],
    },
    {
      title: 'Install dependencies',
      child: [{ title: 'Run npm install in the project root' }, { title: 'Wait for all packages to download and install' }],
    },
    { title: 'Start the development server with npm run dev' },
  ]}
/>
```

**Renders as:**

<ProcessList
  items={[
    {
      title: 'Prepare your development environment',
      child: [
        { title: 'Install Node.js version 18 or later' },
        { title: 'Install Visual Studio Code or your preferred editor' },
        { title: 'Install Git for version control' },
      ],
    },
    {
      title: 'Clone and configure the project',
      child: [
        { title: 'Clone the repository from GitHub' },
        { title: 'Navigate to the project directory' },
        { title: 'Copy .env.example to .env and configure environment variables' },
      ],
    },
    {
      title: 'Install dependencies',
      child: [{ title: 'Run npm install in the project root' }, { title: 'Wait for all packages to download and install' }],
    },
    { title: 'Start the development server with npm run dev' },
  ]}
/>

---

#### Example 3: Expandable Details Panels

Steps with expandable detail sections that can contain rich content:

```jsx
<ProcessList
  items={[
    {
      title: 'Install required software',
      details: (
        <>
          <p>You'll need to install several software packages. Click each link to download:</p>
          <ul>
            <li>
              <a href="https://nodejs.org">Node.js</a> - JavaScript runtime
            </li>
            <li>
              <a href="https://code.visualstudio.com">VS Code</a> - Code editor
            </li>
            <li>
              <a href="https://git-scm.com">Git</a> - Version control
            </li>
          </ul>
          <p>
            <strong>Tip:</strong> Install the LTS (Long Term Support) versions for maximum stability.
          </p>
        </>
      ),
    },
    {
      title: 'Configure your environment',
      details: (
        <>
          <p>Create a .env file in the project root with the following variables:</p>
          <pre>
            {`DATABASE_URL=postgresql://localhost:5432/mydb
API_KEY=your_api_key_here
NODE_ENV=development`}
          </pre>
          <p>
            <strong>Note:</strong> Never commit your .env file to version control!
          </p>
        </>
      ),
    },
    {
      title: 'Run the application',
      details: (
        <>
          <p>Start the development server:</p>
          <pre>npm run dev</pre>
          <p>
            Open your browser to <code>http://localhost:3000</code>
          </p>
        </>
      ),
    },
  ]}
/>
```

**Renders as:**

<ProcessList
  items={[
    {
      title: 'Install required software',
      details: (
        <>
          <p>You'll need to install several software packages. Click each link to download:</p>
          <ul>
            <li>
              <a href="https://nodejs.org">Node.js</a> - JavaScript runtime
            </li>
            <li>
              <a href="https://code.visualstudio.com">VS Code</a> - Code editor
            </li>
            <li>
              <a href="https://git-scm.com">Git</a> - Version control
            </li>
          </ul>
          <p>
            <strong>Tip:</strong> Install the LTS (Long Term Support) versions for maximum stability.
          </p>
        </>
      ),
    },
    {
      title: 'Configure your environment',
      details: (
        <>
          <p>Create a .env file in the project root with the following variables:</p>
          <pre>
            {`DATABASE_URL=postgresql://localhost:5432/mydb
API_KEY=your_api_key_here
NODE_ENV=development`}
          </pre>
          <p>
            <strong>Note:</strong> Never commit your .env file to version control!
          </p>
        </>
      ),
    },
    {
      title: 'Run the application',
      details: (
        <>
          <p>Start the development server:</p>
          <pre>npm run dev</pre>
          <p>
            Open your browser to <code>http://localhost:3000</code>
          </p>
        </>
      ),
    },
  ]}
/>

---

#### Example 4: Combining Sub-steps and Details

Complex workflow with both lettered sub-steps AND expandable details:

```jsx
<ProcessList
  items={[
    {
      title: 'Pre-flight checks',
      child: [
        { title: 'Verify all team members have necessary access' },
        { title: 'Ensure backup systems are operational' },
        { title: 'Review deployment checklist' },
      ],
      details: (
        <>
          <p>
            <strong>Critical:</strong> Do not proceed with deployment until all pre-flight checks pass.
          </p>
          <p>Contact the DevOps team if any checks fail.</p>
        </>
      ),
    },
    {
      title: 'Deploy to staging',
      child: [
        { title: 'Run build process: npm run build' },
        { title: 'Deploy to staging environment' },
        { title: 'Run automated test suite' },
        { title: 'Perform manual smoke tests' },
      ],
      details: (
        <>
          <p>Deployment command:</p>
          <pre>npm run deploy:staging</pre>
          <p>Wait for the deployment to complete (typically 5-10 minutes).</p>
        </>
      ),
    },
    {
      title: 'Deploy to production',
      details: (
        <>
          <p>
            <strong>Warning:</strong> This will deploy to production. Ensure staging tests passed.
          </p>
          <pre>npm run deploy:production</pre>
          <p>Monitor application logs for any errors after deployment.</p>
        </>
      ),
    },
  ]}
/>
```

**Renders as:**

<ProcessList
  items={[
    {
      title: 'Pre-flight checks',
      child: [
        { title: 'Verify all team members have necessary access' },
        { title: 'Ensure backup systems are operational' },
        { title: 'Review deployment checklist' },
      ],
      details: (
        <>
          <p>
            <strong>Critical:</strong> Do not proceed with deployment until all pre-flight checks pass.
          </p>
          <p>Contact the DevOps team if any checks fail.</p>
        </>
      ),
    },
    {
      title: 'Deploy to staging',
      child: [
        { title: 'Run build process: npm run build' },
        { title: 'Deploy to staging environment' },
        { title: 'Run automated test suite' },
        { title: 'Perform manual smoke tests' },
      ],
      details: (
        <>
          <p>Deployment command:</p>
          <pre>npm run deploy:staging</pre>
          <p>Wait for the deployment to complete (typically 5-10 minutes).</p>
        </>
      ),
    },
    {
      title: 'Deploy to production',
      details: (
        <>
          <p>
            <strong>Warning:</strong> This will deploy to production. Ensure staging tests passed.
          </p>
          <pre>npm run deploy:production</pre>
          <p>Monitor application logs for any errors after deployment.</p>
        </>
      ),
    },
  ]}
/>

---

#### Example 5: Using Freeform Content Blocks

Include un-numbered content blocks within a step (doesn't break numbering sequence):

```jsx
<ProcessList
  items={[
    { title: 'Open the software application' },
    {
      title: 'Navigate to File > New Project',
      freeform: (
        <p>
          <strong>Note:</strong> First-time users will be prompted to complete the setup wizard before proceeding.
        </p>
      ),
    },
    { title: 'Enter project details in the dialog box' },
    { title: 'Click Create to initialize the project' },
  ]}
/>
```

**Renders as:**

<ProcessList
  items={[
    { title: 'Open the software application' },
    {
      title: 'Navigate to File > New Project',
      freeform: (
        <p>
          <strong>Note:</strong> First-time users will be prompted to complete the setup wizard before proceeding.
        </p>
      ),
    },
    { title: 'Enter project details in the dialog box' },
    { title: 'Click Create to initialize the project' },
  ]}
/>

---

#### Example 6: Custom Starting Number and Indentation

Control the starting number and indentation levels:

```jsx
<ProcessList
  items={[{ title: 'Review preliminary analysis results' }, { title: 'Prepare final report sections' }, { title: 'Submit for technical review' }]}
  startAt={5}
  indentPx={40}
/>
```

**Renders as:**

<ProcessList
  items={[{ title: 'Review preliminary analysis results' }, { title: 'Prepare final report sections' }, { title: 'Submit for technical review' }]}
  startAt={5}
  indentPx={40}
/>

---

#### Props Reference

**Core Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>items</code>',
      '<code>numberStyle</code>',
      '<code>startAt</code>',
      '<code>indentPx</code>',
      '<code>maxIndentPx</code>',
      '<code>bubbleSizePx</code>',
      '<code>bubbleGapPx</code>',
      '<code>nowrap</code>',
    ],
    ['Array', 'String', 'Number', 'Number', 'Number', 'Number', 'Number', 'Boolean'],
    ['Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    ['-', '<code>"bubble"</code>', '<code>1</code>', '<code>0</code>', 'Auto', '<code>28</code>', '<code>10</code>', '<code>false</code>'],
    [
      'Array of step objects (see Item Object Structure below)',
      'Number display style: <code>"bubble"</code>, <code>"plain"</code>, or <code>"chevron"</code>',
      'Starting number (e.g., 5 starts at &quot;5&quot;)',
      'Indentation per level in pixels',
      'Maximum total indentation (auto: 40% of container width)',
      'Size of numbered circle bubbles in pixels',
      'Gap between bubble and text in pixels',
      'Prevent title text from wrapping',
    ],
  ]}
/>

**Child Layout Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>childBaseIndentPx</code>', '<code>childIndentPx</code>', '<code>childMaxIndentPx</code>'],
    ['Number', 'Number', 'Number'],
    ['No', 'No', 'No'],
    ['<code>15</code>', 'Same as <code>indentPx</code>', 'Auto'],
    ['Base indentation for child sub-steps', 'Indentation increment for nested sub-steps', 'Maximum indentation for sub-steps'],
  ]}
/>

#### Item Object Structure

<TableVerticalNoRef
  alt="Table: Field, Type, Description"
  headers={[[{ value: 'Field' }, { value: 'Type' }, { value: 'Description' }]]}
  columns={[
    ['<code>title</code>', '<code>child</code>', '<code>details</code>', '<code>freeform</code>'],
    ['String/ReactNode', 'Array', 'ReactNode', 'ReactNode'],
    [
      'Step text or JSX content',
      'Array of sub-step objects (renders as a, b, c, etc.)',
      'Expandable details panel with additional content',
      'Un-numbered content block inserted between steps',
    ],
  ]}
/>

**Usage Notes:**

- **title**: Use for the main step description
- **child**: Nest an array of sub-items that will be lettered (a, b, c, etc.)
- **details**: Add expandable panels with rich content (paragraphs, code blocks, lists, etc.)
- **freeform**: Insert callout boxes, warnings, notes between numbered steps without breaking numbering sequence

---

### DocTabs ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Tabbed content panels for organizing alternative content, multi-platform instructions, or version comparisons.

**When to use:**

- Version-specific documentation (different software versions or releases)
- Alternative workflow methods (GUI vs API vs configuration file)
- User role-based content (Administrator, Analyst, End User)
- Different deployment scenarios (standalone vs distributed)
- Tutorial vs reference content
- Configuration examples for different use cases

**Import:**

```jsx
import DocTabs from '@site/src/components/DocTabs';
```

#### Example 1: Version-Specific Documentation

Show different instructions for different software versions:

```jsx
<DocTabs
  items={[
    {
      title: 'Version 3.0',
      label: 'Version 3.0+',
      child: (
        <>
          <p>Configuration for version 3.0 and later:</p>
          <ol>
            <li>Open the Settings dialog</li>
            <li>Navigate to Data Sources → Import</li>
            <li>Select your file format from the dropdown</li>
            <li>Click Import and the software will auto-detect field mappings</li>
          </ol>
          <p>
            <strong>Note:</strong> Version 3.0 includes automatic field mapping.
          </p>
        </>
      ),
    },
    {
      title: 'Version 2.x',
      label: 'Version 2.x',
      child: (
        <>
          <p>Configuration for version 2.x:</p>
          <ol>
            <li>Open File → Import Data</li>
            <li>Browse to your data file</li>
            <li>Manually map each field using the Field Mapper dialog</li>
            <li>Click OK to complete the import</li>
          </ol>
          <p>
            <strong>Note:</strong> Manual field mapping is required in version 2.x.
          </p>
        </>
      ),
    },
    {
      title: 'Legacy',
      label: 'Version 1.x (Legacy)',
      child: (
        <>
          <p>Configuration for legacy version 1.x:</p>
          <p>Edit the configuration file manually:</p>
          <pre>
            {`[DataImport]
SourceFile=C:\\data\\input.csv
FieldMap=field1:col1,field2:col2`}
          </pre>
          <p>Then restart the application to apply changes.</p>
        </>
      ),
    },
  ]}
  defaultValue={0}
/>
```

**Renders as:**

import DocTabs from '@site/src/components/DocTabs';

<DocTabs
  items={[
    {
      title: 'Version 3.0',
      label: 'Version 3.0+',
      child: (
        <>
          <p>Configuration for version 3.0 and later:</p>
          <ol>
            <li>Open the Settings dialog</li>
            <li>Navigate to Data Sources → Import</li>
            <li>Select your file format from the dropdown</li>
            <li>Click Import and the software will auto-detect field mappings</li>
          </ol>
          <p>
            <strong>Note:</strong> Version 3.0 includes automatic field mapping.
          </p>
        </>
      ),
    },
    {
      title: 'Version 2.x',
      label: 'Version 2.x',
      child: (
        <>
          <p>Configuration for version 2.x:</p>
          <ol>
            <li>Open File → Import Data</li>
            <li>Browse to your data file</li>
            <li>Manually map each field using the Field Mapper dialog</li>
            <li>Click OK to complete the import</li>
          </ol>
          <p>
            <strong>Note:</strong> Manual field mapping is required in version 2.x.
          </p>
        </>
      ),
    },
    {
      title: 'Legacy',
      label: 'Version 1.x (Legacy)',
      child: (
        <>
          <p>Configuration for legacy version 1.x:</p>
          <p>Edit the configuration file manually:</p>
          <pre>
            {`[DataImport]
SourceFile=C:\\data\\input.csv
FieldMap=field1:col1,field2:col2`}
          </pre>
          <p>Then restart the application to apply changes.</p>
        </>
      ),
    },
  ]}
  defaultValue={0}
/>

---

#### Example 2: Alternative Workflow Methods

Show different ways to accomplish the same task:

```jsx
<DocTabs
  items={[
    {
      title: 'Interactive',
      label: 'Interactive Workflow',
      child: (
        <>
          <h4>Using the Interactive Interface</h4>
          <ol>
            <li>Click Tools → Run Analysis</li>
            <li>Select your input dataset from the list</li>
            <li>Configure parameters using the visual parameter editor</li>
            <li>Click Run to execute the analysis</li>
            <li>Review results in the Results Viewer</li>
          </ol>
          <p>
            <strong>Best for:</strong> Exploratory analysis, one-off computations, learning the software
          </p>
        </>
      ),
    },
    {
      title: 'Config File',
      label: 'Configuration File',
      child: (
        <>
          <h4>Using Configuration Files</h4>
          <p>Create a configuration file to define your analysis:</p>
          <pre>
            {`# analysis-config.yaml
input:
  dataset: simulation_results.csv
parameters:
  threshold: 0.95
  iterations: 1000
output:
  format: PDF
  location: ./reports/`}
          </pre>
          <p>
            Then execute: <code>run-analysis --config analysis-config.yaml</code>
          </p>
          <p>
            <strong>Best for:</strong> Reproducible workflows, version control, sharing configurations
          </p>
        </>
      ),
    },
    {
      title: 'Scripting',
      label: 'Scripting API',
      child: (
        <>
          <h4>Using the Scripting Interface</h4>
          <p>Automate your workflow with the built-in scripting API:</p>
          <pre>
            {`# analysis-script.py
from software import Analysis

analysis = Analysis()
analysis.load_data("simulation_results.csv")
analysis.set_parameters(threshold=0.95, iterations=1000)
results = analysis.run()
results.export("./reports/output.pdf")`}
          </pre>
          <p>
            <strong>Best for:</strong> Batch processing, automation, integration with other tools
          </p>
        </>
      ),
    },
  ]}
/>
```

**Renders as:**

<DocTabs
  items={[
    {
      title: 'Interactive',
      label: 'Interactive Workflow',
      child: (
        <>
          <h4>Using the Interactive Interface</h4>
          <ol>
            <li>Click Tools → Run Analysis</li>
            <li>Select your input dataset from the list</li>
            <li>Configure parameters using the visual parameter editor</li>
            <li>Click Run to execute the analysis</li>
            <li>Review results in the Results Viewer</li>
          </ol>
          <p>
            <strong>Best for:</strong> Exploratory analysis, one-off computations, learning the software
          </p>
        </>
      ),
    },
    {
      title: 'Config File',
      label: 'Configuration File',
      child: (
        <>
          <h4>Using Configuration Files</h4>
          <p>Create a configuration file to define your analysis:</p>
          <pre>
{`# analysis-config.yaml
input:
  dataset: simulation_results.csv
parameters:
  threshold: 0.95
  iterations: 1000
output:
  format: PDF
  location: ./reports/`}
          </pre>
          <p>Then execute: <code>run-analysis --config analysis-config.yaml</code></p>
          <p>
            <strong>Best for:</strong> Reproducible workflows, version control, sharing configurations
          </p>
        </>
      ),
    },
    {
      title: 'Scripting',
      label: 'Scripting API',
      child: (
        <>
          <h4>Using the Scripting Interface</h4>
          <p>Automate your workflow with the built-in scripting API:</p>
          <pre>
{`# analysis-script.py
from software import Analysis

analysis = Analysis()
analysis.load_data("simulation_results.csv")
analysis.set_parameters(threshold=0.95, iterations=1000)
results = analysis.run()
results.export("./reports/output.pdf")`}

</pre>
<p>
<strong>Best for:</strong> Batch processing, automation, integration with other tools
</p>
</>
),
},
]}
/>

---

#### Example 3: Deployment Scenarios

Show configuration for different deployment types:

```jsx
<DocTabs
  items={[
    {
      title: 'Standalone',
      label: 'Standalone',
      child: (
        <>
          <p>Configuration for standalone deployment:</p>
          <pre>
            {`# config.ini
[Deployment]
Mode=Standalone
Database=./data/local.db

[Performance]
MaxThreads=4
CacheSize=1GB`}
          </pre>
          <p>All data and processing occur on a single machine. Suitable for individual users or small projects.</p>
        </>
      ),
    },
    {
      title: 'Client-Server',
      label: 'Client-Server',
      child: (
        <>
          <p>Configuration for client-server deployment:</p>
          <pre>
            {`# config.ini
[Deployment]
Mode=Client
ServerHost=analysis-server.example.com
ServerPort=8080

[Connection]
Timeout=30
RetryAttempts=3`}
          </pre>
          <p>Multiple clients connect to a central server. Ideal for teams sharing data and computational resources.</p>
        </>
      ),
    },
    {
      title: 'Distributed',
      label: 'Distributed',
      child: (
        <>
          <p>Configuration for distributed deployment:</p>
          <pre>
            {`# config.ini
[Deployment]
Mode=Distributed
ClusterNodes=node1,node2,node3
LoadBalancer=load-balancer.example.com

[DistributedCompute]
PartitionStrategy=DataSize
MaxNodesPerJob=10`}
          </pre>
          <p>Computation distributed across multiple nodes. Designed for large-scale analyses and high-performance computing.</p>
        </>
      ),
    },
  ]}
  equalWidth={true}
  showDividers={true}
/>
```

**Renders as:**

<DocTabs
  items={[
    {
      title: 'Standalone',
      label: 'Standalone',
      child: (
        <>
          <p>Configuration for standalone deployment:</p>
          <pre>
{`# config.ini
[Deployment]
Mode=Standalone
Database=./data/local.db

[Performance]
MaxThreads=4
CacheSize=1GB`}
          </pre>
          <p>All data and processing occur on a single machine. Suitable for individual users or small projects.</p>
        </>
      ),
    },
    {
      title: 'Client-Server',
      label: 'Client-Server',
      child: (
        <>
          <p>Configuration for client-server deployment:</p>
          <pre>
{`# config.ini
[Deployment]
Mode=Client
ServerHost=analysis-server.example.com
ServerPort=8080

[Connection]
Timeout=30
RetryAttempts=3`}
          </pre>
          <p>Multiple clients connect to a central server. Ideal for teams sharing data and computational resources.</p>
        </>
      ),
    },
    {
      title: 'Distributed',
      label: 'Distributed',
      child: (
        <>
          <p>Configuration for distributed deployment:</p>
          <pre>
{`# config.ini
[Deployment]
Mode=Distributed
ClusterNodes=node1,node2,node3
LoadBalancer=load-balancer.example.com

[DistributedCompute]
PartitionStrategy=DataSize
MaxNodesPerJob=10`}

</pre>
<p>Computation distributed across multiple nodes. Designed for large-scale analyses and high-performance computing.</p>
</>
),
},
]}
equalWidth={true}
showDividers={true}
/>

---

#### Example 4: User Role-Based Content

Different instructions based on user role and responsibilities:

```jsx
<DocTabs
  items={[
    {
      title: 'End User',
      label: 'End User',
      value: 'enduser',
      child: (
        <>
          <h4>Running a Simulation</h4>
          <p>As an end user, you can run pre-configured simulations:</p>
          <ol>
            <li>Open the Simulation menu</li>
            <li>Select a simulation template from the library</li>
            <li>Review the input parameters (ask your analyst if you have questions)</li>
            <li>Click Run Simulation</li>
            <li>View results in the Output tab</li>
          </ol>
          <p>
            <strong>Note:</strong> Contact your analyst to create new simulation templates.
          </p>
        </>
      ),
    },
    {
      title: 'Analyst',
      label: 'Analyst',
      value: 'analyst',
      child: (
        <>
          <h4>Creating Simulation Templates</h4>
          <p>As an analyst, you can create and configure simulation templates:</p>
          <ul>
            <li>Design custom simulation scenarios</li>
            <li>Configure input parameters and validation rules</li>
            <li>Set up automated result processing</li>
            <li>Publish templates to the library for end users</li>
            <li>Review and approve simulation results</li>
          </ul>
          <p>
            <strong>Permissions:</strong> Template creation, data analysis, result review
          </p>
        </>
      ),
    },
    {
      title: 'Administrator',
      label: 'Administrator',
      value: 'admin',
      child: (
        <>
          <h4>System Administration</h4>
          <p>As an administrator, you manage the entire system:</p>
          <ul>
            <li>Manage user accounts and role assignments</li>
            <li>Configure system-wide settings and security policies</li>
            <li>Monitor system performance and resource usage</li>
            <li>Set up data connections and integration points</li>
            <li>Manage backup and recovery procedures</li>
            <li>Install updates and maintain the software</li>
          </ul>
          <p>
            <strong>Permissions:</strong> Full system access and configuration
          </p>
        </>
      ),
    },
  ]}
  defaultValue="enduser"
/>
```

**Renders as:**

<DocTabs
  items={[
    {
      title: 'End User',
      label: 'End User',
      value: 'enduser',
      child: (
        <>
          <h4>Running a Simulation</h4>
          <p>As an end user, you can run pre-configured simulations:</p>
          <ol>
            <li>Open the Simulation menu</li>
            <li>Select a simulation template from the library</li>
            <li>Review the input parameters (ask your analyst if you have questions)</li>
            <li>Click Run Simulation</li>
            <li>View results in the Output tab</li>
          </ol>
          <p>
            <strong>Note:</strong> Contact your analyst to create new simulation templates.
          </p>
        </>
      ),
    },
    {
      title: 'Analyst',
      label: 'Analyst',
      value: 'analyst',
      child: (
        <>
          <h4>Creating Simulation Templates</h4>
          <p>As an analyst, you can create and configure simulation templates:</p>
          <ul>
            <li>Design custom simulation scenarios</li>
            <li>Configure input parameters and validation rules</li>
            <li>Set up automated result processing</li>
            <li>Publish templates to the library for end users</li>
            <li>Review and approve simulation results</li>
          </ul>
          <p>
            <strong>Permissions:</strong> Template creation, data analysis, result review
          </p>
        </>
      ),
    },
    {
      title: 'Administrator',
      label: 'Administrator',
      value: 'admin',
      child: (
        <>
          <h4>System Administration</h4>
          <p>As an administrator, you manage the entire system:</p>
          <ul>
            <li>Manage user accounts and role assignments</li>
            <li>Configure system-wide settings and security policies</li>
            <li>Monitor system performance and resource usage</li>
            <li>Set up data connections and integration points</li>
            <li>Manage backup and recovery procedures</li>
            <li>Install updates and maintain the software</li>
          </ul>
          <p>
            <strong>Permissions:</strong> Full system access and configuration
          </p>
        </>
      ),
    },
  ]}
  defaultValue="enduser"
/>

---

#### Example 5: Compact Tabs Without Dividers

Shorter tab labels with custom styling:

```jsx
<DocTabs
  items={[
    {
      title: 'Step1',
      label: 'Step 1',
      child: <p>First, open the application and create a new project.</p>,
    },
    {
      title: 'Step2',
      label: 'Step 2',
      child: <p>Next, import your data files using File → Import.</p>,
    },
    {
      title: 'Step3',
      label: 'Step 3',
      child: <p>Configure analysis parameters in the Settings panel.</p>,
    },
    {
      title: 'Step4',
      label: 'Step 4',
      child: <p>Run the analysis and review the results.</p>,
    },
  ]}
  equalWidth={false}
  showDividers={false}
  defaultValue={0}
/>
```

**Renders as:**

<DocTabs
  items={[
    {
      title: 'Step1',
      label: 'Step 1',
      child: <p>First, open the application and create a new project.</p>,
    },
    {
      title: 'Step2',
      label: 'Step 2',
      child: <p>Next, import your data files using File → Import.</p>,
    },
    {
      title: 'Step3',
      label: 'Step 3',
      child: <p>Configure analysis parameters in the Settings panel.</p>,
    },
    {
      title: 'Step4',
      label: 'Step 4',
      child: <p>Run the analysis and review the results.</p>,
    },
  ]}
  equalWidth={false}
  showDividers={false}
  defaultValue={0}
/>

---

#### Props Reference

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>items</code>', '<code>defaultValue</code>', '<code>equalWidth</code>', '<code>showDividers</code>'],
    ['Array', 'Number/String', 'Boolean', 'Boolean'],
    ['Yes', 'No', 'No', 'No'],
    ['-', '<code>0</code>', '<code>true</code>', '<code>true</code>'],
    [
      'Array of tab objects',
      'Default active tab (0-based index or value string)',
      'Make all tabs equal width',
      'Show vertical dividers between tabs',
    ],
  ]}
/>

#### Tab Object Structure

<TableVerticalNoRef
  alt="Table: Field, Type, Description"
  headers={[[{ value: 'Field' }, { value: 'Type' }, { value: 'Description' }]]}
  columns={[
    ['<code>title</code>', '<code>label</code>', '<code>value</code>', '<code>child</code>'],
    ['String', 'String', 'String', 'ReactNode'],
    [
      'Tab identifier (internal, for accessibility)',
      'Tab display text shown to users',
      'Optional custom value (for defaultValue targeting)',
      'Tab content (can be any JSX/React elements)',
    ],
  ]}
/>

**Best Practices:**

- Use clear, concise tab labels (ideally 1-2 words)
- Keep content length similar across tabs for consistent experience
- Use `defaultValue` to show the most common/recommended option first
- Consider mobile users - fewer, wider tabs work better on small screens
- Don't nest DocTabs within DocTabs (use CollectionList for hierarchical content)

---

### CollectionList ⭐

⭐ **COMMONLY USED COMPONENT**

**Purpose:** Grouped expandable/collapsible items with optional numbering, multiple layout modes, and flexible styling options.

**When to use:**

- FAQs (Frequently Asked Questions)
- Feature lists and software capabilities
- Expandable content groups
- Card-based navigation
- Module or chapter summaries
- Troubleshooting guides

**Import:**

```jsx
import CollectionList from '@site/src/components/CollectionList';
```

#### Example 1: Simple List (No Expandable Items)

List of titles without children - items without a `child` property are not expandable:

```jsx
<CollectionList
  title="Quick Reference"
  items={[
    { title: 'System Requirements: Windows 10 64-bit or later, 8GB RAM minimum' },
    { title: 'Installation Path: C:\\Program Files\\SoftwareName\\' },
    { title: 'Support Email: support@example.com' },
    { title: 'Documentation: https://docs.example.com' },
  ]}
/>
```

**Renders as:**

<CollectionList
  title="Quick Reference"
  items={[
    { title: 'System Requirements: Windows 10 64-bit or later, 8GB RAM minimum' },
    { title: 'Installation Path: C:\\Program Files\\SoftwareName\\' },
    { title: 'Support Email: support@example.com' },
    { title: 'Documentation: https://docs.example.com' },
  ]}
/>

---

#### Example 2: Simple FAQ (First Item Open)

Basic FAQ with the first item expanded by default:

```jsx
<CollectionList
  title="Frequently Asked Questions"
  items={[
    {
      title: 'What is LifeSim?',
      child: (
        <>
          <p>
            LifeSim is a life safety model for estimating loss of life due to dam and levee failures. It combines hydraulic, population, and
            evacuation data to assess consequences.
          </p>
        </>
      ),
    },
    {
      title: 'What platforms are supported?',
      child: <p>LifeSim runs on Windows 10 and later. A 64-bit operating system is required.</p>,
    },
    {
      title: 'How long does a typical analysis take?',
      child: (
        <p>
          Analysis runtime depends on model complexity and grid resolution. Simple models may complete in minutes, while complex scenarios can take
          several hours.
        </p>
      ),
    },
    {
      title: 'Where can I find example projects?',
      child: (
        <>
          <p>Example projects are included with the software installation in the following directory:</p>
          <pre>C:\Program Files\LifeSim\Examples\</pre>
        </>
      ),
    },
  ]}
  defaultOpen="first"
/>
```

**Renders as:**

import CollectionList from '@site/src/components/CollectionList';

<CollectionList
  title="Frequently Asked Questions"
  items={[
    {
      title: 'What is LifeSim?',
      child: (
        <>
          <p>
            LifeSim is a life safety model for estimating loss of life due to dam and levee failures. It combines hydraulic, population, and
            evacuation data to assess consequences.
          </p>
        </>
      ),
    },
    {
      title: 'What platforms are supported?',
      child: <p>LifeSim runs on Windows 10 and later. A 64-bit operating system is required.</p>,
    },
    {
      title: 'How long does a typical analysis take?',
      child: (
        <p>
          Analysis runtime depends on model complexity and grid resolution. Simple models may complete in minutes, while complex scenarios can take
          several hours.
        </p>
      ),
    },
    {
      title: 'Where can I find example projects?',
      child: (
        <>
          <p>Example projects are included with the software installation in the following directory:</p>
          <pre>C:\Program Files\LifeSim\Examples\</pre>
        </>
      ),
    },
  ]}
  defaultOpen="first"
/>

---

#### Example 2: Numbered List (All Items Expanded)

Feature list with decimal numbering and expand/collapse all buttons:

```jsx
<CollectionList
  title="Software Features"
  items={[
    {
      title: 'Hydraulic Routing',
      child: (
        <>
          <p>Advanced flood wave propagation modeling using the full 2D shallow water equations. Includes features for:</p>
          <ul>
            <li>Dam and levee breach modeling</li>
            <li>Terrain-based flow routing</li>
            <li>Time-varying boundary conditions</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Population Analysis',
      child: (
        <>
          <p>Detailed population-at-risk calculations including:</p>
          <ul>
            <li>Census-based population distribution</li>
            <li>Building occupancy analysis</li>
            <li>Time-of-day population variations</li>
            <li>Vulnerable population identification</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Evacuation Modeling',
      child: (
        <>
          <p>Time-dependent evacuation simulations with:</p>
          <ul>
            <li>Multiple evacuation scenarios</li>
            <li>Warning dissemination modeling</li>
            <li>Mobilization time estimates</li>
            <li>Safe destination identification</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Results Visualization',
      child: (
        <>
          <p>Comprehensive visualization tools:</p>
          <ul>
            <li>Interactive 2D flood maps</li>
            <li>Time-series animations</li>
            <li>Statistical summary reports</li>
            <li>Export capabilities for GIS integration</li>
          </ul>
        </>
      ),
    },
  ]}
  autoNumber={true}
  numberStyle="decimal"
  showExpandAll={true}
  defaultOpen="all"
/>
```

**Renders as:**

<CollectionList
  title="Software Features"
  items={[
    {
      title: 'Hydraulic Routing',
      child: (
        <>
          <p>Advanced flood wave propagation modeling using the full 2D shallow water equations. Includes features for:</p>
          <ul>
            <li>Dam and levee breach modeling</li>
            <li>Terrain-based flow routing</li>
            <li>Time-varying boundary conditions</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Population Analysis',
      child: (
        <>
          <p>Detailed population-at-risk calculations including:</p>
          <ul>
            <li>Census-based population distribution</li>
            <li>Building occupancy analysis</li>
            <li>Time-of-day population variations</li>
            <li>Vulnerable population identification</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Evacuation Modeling',
      child: (
        <>
          <p>Time-dependent evacuation simulations with:</p>
          <ul>
            <li>Multiple evacuation scenarios</li>
            <li>Warning dissemination modeling</li>
            <li>Mobilization time estimates</li>
            <li>Safe destination identification</li>
          </ul>
        </>
      ),
    },
    {
      title: 'Results Visualization',
      child: (
        <>
          <p>Comprehensive visualization tools:</p>
          <ul>
            <li>Interactive 2D flood maps</li>
            <li>Time-series animations</li>
            <li>Statistical summary reports</li>
            <li>Export capabilities for GIS integration</li>
          </ul>
        </>
      ),
    },
  ]}
  autoNumber={true}
  numberStyle="decimal"
  showExpandAll={true}
  defaultOpen="all"
/>

---

#### Example 4: Grid Layout with Cards (No Expandable Items)

Card-based grid layout with titles only (no `child` property means no expand/collapse):

```jsx
<CollectionList
  title="Analysis Modules"
  items={[
    { title: 'Consequence Estimation' },
    { title: 'Risk Assessment' },
    { title: 'Uncertainty Analysis' },
    { title: 'Sensitivity Analysis' },
    { title: 'Portfolio Analysis' },
    { title: 'Decision Analysis' },
  ]}
  layout="grid"
  columns={{ base: 1, sm: 2, md: 3, xl: 3 }}
/>
```

**Renders as:**

<CollectionList
  title="Analysis Modules"
  items={[
    { title: 'Consequence Estimation' },
    { title: 'Risk Assessment' },
    { title: 'Uncertainty Analysis' },
    { title: 'Sensitivity Analysis' },
    { title: 'Portfolio Analysis' },
    { title: 'Decision Analysis' },
  ]}
  layout="grid"
  columns={{ base: 1, sm: 2, md: 3, xl: 3 }}
/>

---

#### Example 5: Different Numbering Styles

Demonstrate various numbering formats:

```jsx
<CollectionList
  title="Roman Numeral Numbering"
  items={[
    { title: 'Introduction', child: 'Overview of the methodology and approach.' },
    { title: 'Data Requirements', child: 'Description of required input data and sources.' },
    { title: 'Analysis Procedures', child: 'Step-by-step analysis instructions.' },
  ]}
  autoNumber={true}
  numberStyle="upper-roman"
  defaultOpen="none"
/>

</br>

<CollectionList
  title="Alphabetic Numbering"
  items={[
    { title: 'Scenario A: Sunny Day Failure', child: 'Failure occurs during normal operations with no precipitation.' },
    { title: 'Scenario B: PMF Event', child: 'Failure during Probable Maximum Flood event.' },
    { title: 'Scenario C: Seismic Event', child: 'Failure triggered by maximum credible earthquake.' },
  ]}
  autoNumber={true}
  numberStyle="upper-alpha"
  startAt={1}
/>
```

**Renders as:**

<CollectionList
  title="Roman Numeral Numbering"
  items={[
    { title: 'Introduction', child: 'Overview of the methodology and approach.' },
    { title: 'Data Requirements', child: 'Description of required input data and sources.' },
    { title: 'Analysis Procedures', child: 'Step-by-step analysis instructions.' },
  ]}
  autoNumber={true}
  numberStyle="upper-roman"
  defaultOpen="none"
/>

<p></p>

<CollectionList
  title="Alphabetic Numbering"
  items={[
    { title: 'Scenario A: Sunny Day Failure', child: 'Failure occurs during normal operations with no precipitation.' },
    { title: 'Scenario B: PMF Event', child: 'Failure during Probable Maximum Flood event.' },
    { title: 'Scenario C: Seismic Event', child: 'Failure triggered by maximum credible earthquake.' },
  ]}
  autoNumber={true}
  numberStyle="upper-alpha"
  startAt={1}
/>

---

#### Example 6: Specific Items Open by Default

Control exactly which items are expanded using index array:

```jsx
<CollectionList
  title="Chapter Summary"
  items={[
    {
      title: 'Chapter 1: Getting Started',
      child: 'Introduction to the software and basic concepts.',
    },
    {
      title: 'Chapter 2: Data Preparation',
      child: 'How to prepare and import data for analysis.',
    },
    {
      title: 'Chapter 3: Running Analyses',
      child: 'Step-by-step instructions for running various analysis types.',
    },
    {
      title: 'Chapter 4: Interpreting Results',
      child: 'Understanding and visualizing analysis outputs.',
    },
    {
      title: 'Chapter 5: Advanced Topics',
      child: 'Advanced features for experienced users.',
    },
  ]}
  defaultOpen={[1, 3]}
  autoNumber={true}
/>
```

**Renders as:**

<CollectionList
  title="Chapter Summary"
  items={[
    {
      title: 'Chapter 1: Getting Started',
      child: 'Introduction to the software and basic concepts.',
    },
    {
      title: 'Chapter 2: Data Preparation',
      child: 'How to prepare and import data for analysis.',
    },
    {
      title: 'Chapter 3: Running Analyses',
      child: 'Step-by-step instructions for running various analysis types.',
    },
    {
      title: 'Chapter 4: Interpreting Results',
      child: 'Understanding and visualizing analysis outputs.',
    },
    {
      title: 'Chapter 5: Advanced Topics',
      child: 'Advanced features for experienced users.',
    },
  ]}
  defaultOpen={[1, 3]}
  autoNumber={true}
/>

---

#### Props Reference

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>title</code>',
      '<code>items</code>',
      '<code>startAt</code>',
      '<code>autoNumber</code>',
      '<code>numberStyle</code>',
      '<code>defaultOpen</code>',
      '<code>showExpandAll</code>',
      '<code>layout</code>',
      '<code>columns</code>',
    ],
    ['String', 'Array', 'Number', 'Boolean', 'String', 'String/Array', 'Boolean', 'String', 'Object'],
    ['No', 'Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    [
      '-',
      '-',
      '<code>1</code>',
      '<code>false</code>',
      '<code>&quot;decimal&quot;</code>',
      '<code>&quot;none&quot;</code>',
      '<code>false</code>',
      '<code>&quot;stack&quot;</code>',
      '-',
    ],
    [
      'Optional title displayed above the list',
      'Array of item objects (see below)',
      'Starting number for numbering (if autoNumber is true)',
      'Enable automatic numbering',
      'Numbering format: <code>&quot;decimal&quot;</code>, <code>&quot;upper-alpha&quot;</code>, <code>&quot;lower-alpha&quot;</code>, <code>&quot;upper-roman&quot;</code>, <code>&quot;lower-roman&quot;</code>',
      'Control which items are expanded: <code>&quot;none&quot;</code>, <code>&quot;first&quot;</code>, <code>&quot;all&quot;</code>, or array of indices (e.g., <code>[0, 2]</code>)',
      'Show expand all / collapse all buttons',
      'Layout mode: <code>&quot;stack&quot;</code> (vertical) or <code>&quot;grid&quot;</code> (cards)',
      'Grid columns for different breakpoints: <code>{ base: 1, sm: 2, md: 3, xl: 4 }</code>',
    ],
  ]}
/>

#### Item Object Structure

<TableVerticalNoRef
  alt="Table: Field, Type, Description"
  headers={[[{ value: 'Field' }, { value: 'Type' }, { value: 'Description' }]]}
  columns={[
    ['<code>title</code>', '<code>child</code>'],
    ['String/ReactNode', 'String/ReactNode'],
    ['Item title/heading (always visible)', 'Item content (shown when expanded or if defaultOpen includes this item)'],
  ]}
/>

**Best Practices:**

- **FAQs**: Can use `defaultOpen="first"` to show users there's expandable content
- **Feature lists**: Can use `defaultOpen="all"` with `showExpandAll={true}` for easy navigation
- **Module navigation**: Can use `layout="grid"` with appropriate column counts
- **Always-visible content**: Set `defaultOpen="all"` and `showExpandAll={false}`
- Keep item titles concise (under 10 words)
- Use rich JSX content in `child` for better formatting

---

## Multimedia

### Video

**Purpose:** Numbered video component with automatic numbering.

**When to use:**

- Tutorial videos
- Demonstration videos
- Multimedia documentation

**Import:**

```jsx
import Video from '@site/src/components/Video';
```

**Basic Usage:**

```jsx
<Video
  videoKey="video-tutorial"
  caption="Software installation tutorial"
  src="/videos/installation-tutorial.mp4"
  poster="/videos/installation-poster.jpg"
  width="90%"
/>
```

**Multiple Sources:**

```jsx
<Video
  videoKey="video-demo"
  caption="Feature demonstration"
  sources={[
    { src: '/videos/demo.mp4', type: 'video/mp4' },
    { src: '/videos/demo.webm', type: 'video/webm' },
  ]}
  poster="/videos/demo-poster.jpg"
  controls={true}
/>
```

**With Captions:**

```jsx
<Video
  videoKey="video-presentation"
  caption="Conference presentation"
  src="/videos/presentation.mp4"
  trackSrc="/videos/presentation-captions.vtt"
  trackLang="en"
  trackLabel="English"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    [
      '<code>videoKey</code>',
      '<code>caption</code>',
      '<code>poster</code>',
      '<code>src</code>',
      '<code>sources</code>',
      '<code>width</code>',
      '<code>controls</code>',
      '<code>autoPlay</code>',
      '<code>loop</code>',
      '<code>muted</code>',
      '<code>playsInline</code>',
      '<code>preload</code>',
      '<code>trackSrc</code>',
      '<code>trackLang</code>',
      '<code>trackLabel</code>',
      '<code>credit</code>',
    ],
    [
      'String',
      'String',
      'String',
      'String',
      'Array',
      'String',
      'Boolean',
      'Boolean',
      'Boolean',
      'Boolean',
      'Boolean',
      'String',
      'String',
      'String',
      'String',
      'String',
    ],
    ['Yes', 'Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    [
      '-',
      '-',
      '-',
      '-',
      '<code>[]</code>',
      '<code>&quot;80%&quot;</code>',
      '<code>true</code>',
      '<code>false</code>',
      '<code>false</code>',
      'Auto',
      '<code>true</code>',
      '<code>&quot;metadata&quot;</code>',
      '-',
      '<code>&quot;en&quot;</code>',
      '<code>&quot;English&quot;</code>',
      '-',
    ],
    [
      'Unique identifier',
      'Video caption',
      'Poster image path',
      'Single video source',
      'Multiple sources <code>[{src, type}]</code>',
      'Display width',
      'Show video controls',
      'Auto-play on load',
      'Loop playback',
      'Mute audio (auto=true if autoPlay)',
      'iOS inline playback',
      'Preload strategy',
      'VTT captions file',
      'Caption language',
      'Caption label',
      'Video source/credit',
    ],
  ]}
/>

---

### VideoReference

**Purpose:** Cross-references to numbered videos.

**When to use:**

- Referencing videos from text

**Import:**

```jsx
import VideoReference from '@site/src/components/VideoReference';
```

**Usage:**

```jsx
See <VideoReference videoKey="video-tutorial" /> for installation instructions.
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>videoKey</code>'], ['String'], ['Yes'], ['Must match a Video&#x27;s videoKey']]}
/>

---

## Navigation

### Button

**Purpose:** Styled button or link.

**When to use:**

- Call-to-action links
- Navigation buttons
- Download buttons

**Import:**

```jsx
import Button from '@site/src/components/Button';
```

**Link Button:**

```jsx
<Button href="/docs/getting-started">Get Started</Button>
```

**External Link:**

```jsx
<Button href="https://example.com">Visit Website</Button>
```

**Click Handler:**

```jsx
<Button onClick={() => alert('Clicked!')}>Click Me</Button>
```

Renders as:

<Button onClick={() => alert('Clicked!')}>Click Me</Button>

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>children</code>', '<code>href</code>', '<code>to</code>', '<code>onClick</code>', '<code>className</code>'],
    ['ReactNode', 'String', 'String', 'Function', 'String'],
    ['Yes', 'No', 'No', 'No', 'No'],
    [
      'Button text/content',
      'Link URL (renders as <code>&lt;a&gt;</code>)',
      'Internal link (alias for href)',
      'Click handler (renders as <code>&lt;button&gt;</code>)',
      'Additional CSS classes',
    ],
  ]}
/>

---

### NavLink

**Purpose:** Styled navigation link with arrow.

**When to use:**

- Section navigation
- Included automatically in NavContainer
- Rarely used standalone

**Import:**

```jsx
import NavLink from '@site/src/components/NavLink';
```

**Usage:** Typically used internally by NavContainer.

---

### NavContainer

**Purpose:** Navigation layout with version selector.

**When to use:**

- Page navigation sections
- Version-aware navigation

**Import:**

```jsx
import NavContainer from '@site/src/components/NavContainer';
```

**Usage:**

```jsx
<NavContainer
  link="/desktop-applications/lifesim"
  linkTitle="User's Guide"
  document="desktop-applications/lifesim/users-guide"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>link</code>', '<code>linkTitle</code>', '<code>document</code>'],
    ['String', 'String', 'String'],
    ['Yes', 'Yes', 'Yes'],
    ['Parent link URL', 'Navigation title', 'Document identifier for version resolution'],
  ]}
/>

---

### VersionSelector

**Purpose:** Dropdown for switching document versions.

**When to use:**

- Included automatically in NavContainer
- Rarely used standalone

**Import:**

```jsx
import VersionSelector from '@site/src/components/VersionSelector';
```

**Usage:** Typically used internally by NavContainer.

---

## Specialized Components

### EventTree

**Purpose:** Displays an event tree image with an optional download link to the associated spreadsheet. Used extensively in the Typical Event Tree Database.

**When to use:**

- Typical Event Tree Database pages
- Displaying event tree diagrams with download links

**Import:**

```jsx
import EventTree from '@site/src/components/EventTree';
```

**Usage:**

```jsx
<EventTree base="embankment-overtopping" />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>base</code>', '<code>text</code>', '<code>imgExt</code>', '<code>sheetExt</code>', '<code>imgDir</code>', '<code>sheetDir</code>', '<code>width</code>', '<code>pngScale</code>'],
    ['String', 'String', 'String', 'String', 'String', 'String', 'String', 'Number'],
    ['Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'],
    ['-', '-', '<code>"png"</code>', '<code>"xlsm"</code>', '<code>"/event-trees/images"</code>', '<code>"/event-trees/spreadsheets"</code>', '<code>"100%"</code>', '<code>4</code>'],
    ['Base filename for the event tree image and spreadsheet', 'Display text', 'Image file extension', 'Spreadsheet file extension', 'Directory path for images', 'Directory path for spreadsheets', 'Image width', 'Scale factor for SVG-to-PNG rasterization'],
  ]}
/>

:::tip
In most cases, you only need the `base` prop. The component automatically constructs image and spreadsheet paths from the base filename.
:::

---

### EventTreeTOC

**Purpose:** Renders an auto-generated table of contents for the Typical Event Tree Database. Reads TOC data from `src/data/eventTreeToc.json`.

**When to use:**

- Table of contents pages in the Typical Event Tree Database

**Import:**

```jsx
import EventTreeTOC from '@site/src/components/EventTreeTOC';
```

**Usage:**

```jsx
<EventTreeTOC />
```

**Props:** None — reads the current URL path to determine the version and loads data automatically.

---

### CategoryList

**Purpose:** Renders a styled list of category items with optional icons, descriptions, and badges. Supports multiple layout variants.

**When to use:**

- Displaying categorized navigation or feature lists
- Tool or application listings with descriptions

**Import:**

```jsx
import CategoryList from '@site/src/components/CategoryList';
```

**Usage:**

```jsx
<CategoryList
  items={[
    { title: 'Getting Started', description: 'Learn the basics', href: './getting-started' },
    { title: 'Advanced Topics', description: 'Deep dive into features', href: './advanced' },
  ]}
  variant="accentTiles"
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>items</code>', '<code>variant</code>', '<code>columns</code>'],
    ['Array', 'String', 'Object'],
    ['Yes', 'No', 'No'],
    ['<code>[]</code>', '<code>"accentTiles"</code>', '<code>{`{base: 1, md: 2, xl: 3}`}</code>'],
    ['Array of <code>{`{title, description?, href?, onClick?, icon?, badge?}`}</code> objects', 'Layout variant: <code>"accentTiles"</code>, <code>"pills"</code>, or <code>"menu"</code>', 'Grid column configuration for accentTiles variant'],
  ]}
/>

---

### TableBestFitDistributions

**Purpose:** Specialized table component for displaying distribution comparison data in RMC-BestFit documentation. Supports header config with column spans.

**When to use:**

- RMC-BestFit probability distribution comparison tables

**Import:**

```jsx
import TableBestFitDistributions from '@site/src/components/TableBestFitDistributions';
```

**Usage:**

```jsx
<TableBestFitDistributions
  headers={['Distribution', 'Parameters', 'AIC', 'BIC']}
  columns={[
    ['Normal', 'Log-Normal', 'Weibull'],
    ['μ=5.2, σ=1.1', 'μ=1.6, σ=0.4', 'k=3.2, λ=5.8'],
    ['125.3', '118.7', '121.2'],
    ['129.1', '122.5', '125.0'],
  ]}
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Default, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Default' }, { value: 'Description' }]]}
  columns={[
    ['<code>headerConfig</code>', '<code>headers</code>', '<code>columns</code>', '<code>fullWidth</code>', '<code>colWidths</code>', '<code>colAlign</code>', '<code>widthMode</code>'],
    ['Array', 'Array', 'Array', 'Boolean', 'Array', 'Array', 'String'],
    ['No', 'No', 'Yes', 'No', 'No', 'No', 'No'],
    ['-', '-', '-', '<code>true</code>', '-', '-', '<code>"full"</code>'],
    ['Header config with <code>colSpan</code> support', 'Plain string headers (fallback)', 'Column data arrays', 'Whether table spans full width', 'Column width specifications', 'Column alignment specifications', 'Width mode: <code>"full"</code> or <code>"intrinsic"</code>'],
  ]}
/>

---

## Schema Components (DST Database)

These components are used exclusively in the DST database schema documentation (`docs/dev/dst/database-schema/`).

### SchemaTableCompact

**Purpose:** Compact table displaying a database table's schema — column names, types, constraints, and optional grouping.

**Import:**

```jsx
import SchemaTableCompact from '@site/src/components/SchemaTableCompact';
```

**Usage:**

```jsx
<SchemaTableCompact
  name="Locations"
  description="Stores geographic location data."
  status="implemented"
  columns={[
    { name: 'LocationID', type: 'int', pk: true, description: 'Primary key' },
    { name: 'Name', type: 'nvarchar(200)', description: 'Location name' },
  ]}
/>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>name</code>', '<code>description</code>', '<code>status</code>', '<code>columns</code>', '<code>notes</code>', '<code>jsonSchemas</code>', '<code>id</code>'],
    ['String', 'String', 'String', 'Array', 'String', 'Array', 'String'],
    ['Yes', 'No', 'No', 'Yes', 'No', 'No', 'No'],
    ['Table name', 'Table description', '<code>"implemented"</code> (default), <code>"partial"</code>, or <code>"planned"</code>', 'Array of column definition objects', 'Additional notes', 'JSON schema examples', 'Custom HTML id'],
  ]}
/>

---

### SchemaTag

**Purpose:** Inline colored tag/badge for labeling schema elements (e.g., primary keys, foreign keys, data types).

**Import:**

```jsx
import SchemaTag from '@site/src/components/SchemaTag';
```

**Usage:**

```jsx
<SchemaTag type="pk">Primary Key</SchemaTag>
<SchemaTag type="fk" href="#locations-table">LocationID</SchemaTag>
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[
    ['<code>type</code>', '<code>children</code>', '<code>href</code>', '<code>title</code>'],
    ['String', 'Node', 'String', 'String'],
    ['Yes', 'Yes', 'No', 'No'],
    ['Tag style type (e.g., <code>"pk"</code>, <code>"fk"</code>)', 'Label content', 'If provided, renders as a link', 'Tooltip text'],
  ]}
/>

---

### SchemaRelationshipDiagram

**Purpose:** Data-driven entity relationship diagram rendered with CSS flexbox, showing table relationships within DST database modules.

**Import:**

```jsx
import SchemaRelationshipDiagram from '@site/src/components/SchemaRelationshipDiagram';
```

**Usage:**

```jsx
{/* Show all modules and relationships */}
<SchemaRelationshipDiagram />

{/* Filter to a single module */}
<SchemaRelationshipDiagram moduleId="common" />
```

**Props:**

<TableVerticalNoRef
  alt="Table: Prop, Type, Required, Description"
  headers={[[{ value: 'Prop' }, { value: 'Type' }, { value: 'Required' }, { value: 'Description' }]]}
  columns={[['<code>moduleId</code>'], ['String'], ['No'], ['Filter diagram to a single module. When omitted, all modules are shown.']]}
/>

---

## Component Best Practices

### General Guidelines

**Always provide required props:**

```jsx
✅ Good:
<Figure
  figKey="figure-1"
  src="/figures/image.png"
  alt="Detailed description"
  caption="Informative caption"
/>

❌ Bad:
<Figure src="/figures/image.png" />  // Missing required props
```

**Use unique keys:**

```jsx
✅ Good:
<Figure figKey="figure-3" ... />
<Figure figKey="figure-6" ... />

❌ Bad:
<Figure figKey="figure-1" ... />
<Figure figKey="figure-1" ... />  // Duplicate key!
```

**Write descriptive alt text:**

```jsx
✅ Good:
alt="Bar chart comparing baseline, alternative A, and alternative B scenarios"

❌ Bad:
alt="Chart"
alt="Figure 1"
```

### Accessibility

**All images need alt text:**

- Describe what's shown in the image
- Don't say "image of" or "picture of"
- Be specific and concise

**Use semantic structure:**

- Headings in logical order
- Proper table markup
- Meaningful link text

### Performance

**Optimize images:**

- Keep file sizes reasonable (< 500KB each)
- Use appropriate formats (PNG for diagrams, JPG for photos)
- Compress images before adding

**Lazy loading:**

- Images and videos load automatically as needed
- No action required from contributors

### Common Mistakes

**Forgetting imports:**

```jsx
❌ Error: Component not defined
// Forgot to import Figure

✅ Fix:
import Figure from '@site/src/components/Figure';
```

**Wrong path format:**

```jsx
❌ Bad:
src="figures/image.png"  // Missing leading slash
src="../figures/image.png"  // Relative path

✅ Good:
src="/figures/desktop-applications/software/v1.0/image.png"
```

**Mismatched keys:**

```jsx
❌ Bad:
<Figure figKey="figure-3" ... />
...
<FigureReference figKey="figure-7" />  // Typo!

✅ Good:
<Figure figKey="figure-3" ... />
...
<FigureReference figKey="figure-3" />
```

---

## Troubleshooting

### Component Not Rendering

**Problem:** Component doesn't appear on page.

**Solutions:**

1. Check import statement is correct
2. Verify all required props are provided
3. Check for typos in component name
4. Look in browser console for error messages (if able; USG machines do not typically allow access to browser console)

### Numbers Are Wrong

**Problem:** Figure/table/equation numbers are incorrect.

**Solutions:**

1. Check for duplicate keys
2. Verify files are numbered correctly (`01-`, `02-`, etc.)
3. Restart dev server (Ctrl + C -> `npm start`) to trigger the `counters` script during build

### Cross-References Broken

**Problem:** References show "???" or don't link.

**Solutions:**

1. Ensure referenced key exists
2. Check for typos in keys
3. Verify component is on a page that's been processed

### Images Not Displaying

**Problem:** Images show broken icon.

**Solutions:**

1. Check path starts with `/figures/`
2. Verify image exists at specified path
3. Check filename matches exactly (case-sensitive)
4. Ensure image format is supported (PNG, JPG, SVG)

---

## Quick Reference Table

<TableVerticalNoRef
  alt="Table: Need to..., Use Component, Import"
  headers={[[{ value: 'Need to...' }, { value: 'Use Component' }, { value: 'Import' }]]}
  columns={[
    [
      'Show numbered figure',
      'Reference a figure',
      'Show numbered table',
      'Show numbered equation',
      'Add citation',
      'Step-by-step process',
      'Tabbed content',
      'Expandable list',
      'Video with caption',
      'Navigation button',
      'Document metadata',
    ],
    [
      '<code>&lt;Figure&gt;</code>',
      '<code>&lt;FigureReference&gt;</code>',
      '<code>&lt;TableVertical&gt;</code>',
      '<code>&lt;Equation&gt;</code>',
      '<code>&lt;Citation&gt;</code>',
      '<code>&lt;ProcessList&gt;</code>',
      '<code>&lt;DocTabs&gt;</code>',
      '<code>&lt;CollectionList&gt;</code>',
      '<code>&lt;Video&gt;</code>',
      '<code>&lt;Button&gt;</code>',
      '<code>&lt;DocumentMetadata&gt;</code>',
    ],
    [
      '<code>@site/src/components/Figure</code>',
      '<code>@site/src/components/FigureReference</code>',
      '<code>@site/src/components/TableVertical</code>',
      '<code>@site/src/components/Equation</code>',
      '<code>@site/src/components/Citation</code>',
      '<code>@site/src/components/ProcessList</code>',
      '<code>@site/src/components/DocTabs</code>',
      '<code>@site/src/components/CollectionList</code>',
      '<code>@site/src/components/Video</code>',
      '<code>@site/src/components/Button</code>',
      '<code>@site/src/components/DocumentMetadata</code>',
    ],
  ]}
/>

---

## Next Steps

Now that you understand the available components:

1. **Practice:** Try using components in a test MDX file
2. **Explore:** Look at existing documentation for real-world examples
3. **Experiment:** Test different props and configurations
4. **Reference:** Bookmark this page for quick lookup

**Related guides:**

- [Creating and Editing Pages](./06-creating-editing-pages.mdx) - MDX syntax and structure
- [Project Structure](./03-project-structure.mdx) - Where to place images and files

**Need help?** See [Troubleshooting & FAQ](./08-troubleshooting-faq.mdx) for solutions to common issues.


