...
:::tip Batch your replies
Answer threads from the **Files changed** tab using **Start a review**, then submit once when you have worked through them all. Replying from the Conversation tab posts each reply as its own standalone review, and every one of those sends a separate email to everyone following the PR. See [Reviewer Workflow §5 — Replying to comments](./12-reviewer-workflow.mdx#replying-to-comments-and-follow-up-discussion).
:::
...
```
- [x] Technical edit comments addressed
```

Checking this box advances the document to the next stage — **Director review for Lane 1 only**. Lanes 2 and 3 have no Director review; checking the box moves them straight to `stage:ready-to-merge`. The author does not check the box until every comment has been addressed.
