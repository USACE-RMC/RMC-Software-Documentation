...
**Manual override.** If a human technical editor performed the review instead, or the author is unavailable to check the checkbox, the site administrator applies the `admin:technical-edit-complete` label to the PR. The workflow removes the label and advances the stage the same as a checkbox event, then posts the next-stage instructions — Director review for Lane 1, ready-to-merge for Lanes 2 and 3. (The older label name `admin:advance-to-director` is still accepted as an alias, but it only ever advanced Lane 1 to a Director and is being retired.)
