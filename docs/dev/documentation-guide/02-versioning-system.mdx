...
The `main` branch is protected — changes cannot be pushed directly. All revisions are committed to a feature branch and merged via a pull request. The branch prefix routes the PR to the appropriate review lane (see [Review Lanes](./10-review-lanes.mdx)):
      ...
      child: (
        <>
          ...
          <ol>
            ...
            <li>Fill in the PR description, assign a peer reviewer if known, and click <strong>Create pull request</strong></li>
          </ol>
        </>
      ),
    },
...
After the PR is opened, a preview build is published to an unadvertised URL and the review workflow auto-assigns a lane label. See [Author Workflow](./11-author-workflow.mdx) for what happens next.
