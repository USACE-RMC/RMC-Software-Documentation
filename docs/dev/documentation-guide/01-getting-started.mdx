...
:::tip Deployment is Admin-Only
Contributors should focus on creating and editing documentation content using `npm start` for local testing. Site administrators will handle all building and deployment to production. See [Appendix B: Build Process Overview](./10-appendix-b-build-process-overview.mdx) for deployment details.
:::

:::tip Take the Guided Site Tour
Once the site is running, visit the homepage and click **"Take a tour"** for an interactive walkthrough of the site's key features — navigation, search, sidebar, document info, figures, citations, and more. It's the fastest way to get oriented before you start contributing.
:::
...
If you encounter issues not covered here, see the [Troubleshooting & FAQ](./08-troubleshooting-faq.mdx) section for more detailed guidance.
