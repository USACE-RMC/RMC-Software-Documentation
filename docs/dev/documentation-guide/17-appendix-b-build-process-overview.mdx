...
npm start
    ↓
1. Generate Image Dimensions
    ↓
2. Generate Report ID Map
    ↓
3. Generate Sidebars
    ↓
4. Generate Counters
...
5. Generate Versions
    ↓
6. Generate Event Tree TOC
    ↓
7. Start Docusaurus Dev Server
    ↓
...
## The Six Automation Scripts

### 1. Image Dimensions Generator

**Script:** `scripts/generateImageDimensions.js`

**What it does:**

- Scans all image files referenced in the documentation
- Extracts width and height metadata for each image
- Generates a JavaScript module with dimension data

**Output:** `src/imageDimensions.js` (auto-generated)

**Why it matters:**

- Enables responsive image rendering with correct aspect ratios
- Prevents layout shift by reserving space before images load
- Used by Figure components for proper image sizing

---

### 2. Report ID Map Generator
...
2. Encounters first `<Figure figKey="figure-3">` → assigns Figure 1
3. Encounters second `<Figure figKey="figure-6">` → assigns Figure 2
...
<TableVerticalNoRef
  ...
  columns={[
    ['<code>src/imageDimensions.js</code>', '<code>src/reportIdMap.js</code>', '<code>sidebars.js</code>', '<code>static/counters/*.json</code>', '<code>static/versions/*.json</code>', '<code>src/data/eventTreeToc.json</code>', '<code>src/data/dstSchemaData.js</code>', '<code>.docusaurus/</code>', '<code>build/</code>'],
    ['generateImageDimensions.js', 'generateReportIdMap.js', 'generateSidebars.js', 'counters.js', 'versions.js', 'generateEventTreeTOC.js', 'Build process', 'Docusaurus', 'Docusaurus'],
    ['Image dimension metadata', 'Document ID mappings', 'Navigation structure', 'Figure/table/equation numbering', 'Version management', 'Event tree navigation', 'DST schema data', 'Build cache', 'Production output'],
  ]}
/>
...
After the six scripts complete, Docusaurus takes over:

### Development Mode (`npm start`)

1. **Pre-build phase:**
   - Runs all 6 automation scripts
...
```bash
npm start      # Runs all 6 scripts, then starts dev server
npm run build  # Runs all 6 scripts, then builds for production
```
...
Contributors use `npm start` while editing and should run `npm run build` when their changes can affect the site. Pull-request builds and previews do not receive production publishing credentials.

Production builds automatically only from `main`. Only administrators merge to `main` and approve the protected production environment. Do not run `npm run deploy` from an author, Director Review, or Publication PR branch.
...
- Runs all 6 automation scripts (Report ID Map, Sidebars, Counters, Versions, Event Tree TOC)
...
This serves the production build locally (typically at `http://localhost:3000`).
...
After local verification, push the branch and use the review workflow. An eligible PR merged by an administrator triggers the production build from the exact `main` revision and waits for environment approval.

### Post-Deployment Verification

After the approved deployment succeeds, verify the live site:

1. **Visit the production URL:** [https://software.rmc.usace.army.mil](https://software.rmc.usace.army.mil) (or your configured domain)
...
- Test production build locally with `npx serve build`
...
- ✓ Test all changes locally with `npm start`
...
If deployment fails, preserve completed review state and any Director preview. Correct the problem through a PR to `main`, then let an administrator retry production. Do not automatically restart reviews, publish a historical commit directly, or delete the stable preview.
...
- **Quick troubleshooting:** [Troubleshooting & FAQ](./08-troubleshooting-faq.mdx)
