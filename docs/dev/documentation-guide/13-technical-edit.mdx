...
<TableVerticalNoRef
  alt="When the technical edit runs in each lane"
  headers={[[{ value: 'Lane' }, { value: 'Trigger point' }]]}
  columns={[
    [
      'Lane 1 (new document)',
...
<ProcessList
  items={[
    { title: 'A team member with access to the AI tooling checks out the PR branch and runs the technical edit.' },
    { title: 'The tooling reads all changed MDX files and applies a standardized review prompt committed to the repository at .github/ai-review/technical-editor-prompt.md. The prompt is reproduced verbatim at the bottom of this chapter.' },
    { title: 'The AI posts inline review comments directly on the PR — on specific lines, with severity levels and suggested fixes — exactly like a human reviewer would.' },
    { title: 'The author addresses each comment by pushing fixes, replying to threads, and resolving conversations as they are addressed. A technical edit is the highest-volume round of the whole workflow, so batch the replies — see the note below.' },
    { title: 'When all comments are addressed, the author checks the checkbox in the PR description: Technical edit comments addressed.' },
    { title: 'The stage progression workflow detects the checkbox and advances the PR to the next stage.' },
  ]}
  numberStyle="plain"
/>

:::tip Reply in a batch, not one at a time
A technical edit routinely produces fifty or more inline comments. Answer them from the **Files changed** tab with **Start a review** and submit once at the end: that is one notification instead of one per reply. Replying from the Conversation tab files each reply as its own standalone review and emails every subscriber each time. See [Reviewer Workflow §5 — Replying to comments](./12-reviewer-workflow.mdx#replying-to-comments-and-follow-up-discussion).
:::

Checking the box advances Lane 1 to Director review. Lanes 2 and 3 have no Director review — they advance straight to `stage:ready-to-merge`.

If the technical edit was performed by a human, or the author is unavailable to check the box, a site administrator can apply the `admin:technical-edit-complete` label to the PR as a manual override. The workflow removes the label and advances the stage the same as a checkbox event. (The older label name `admin:advance-to-director` still works, but it is misleading outside Lane 1 and is being retired.)
...
<TableVerticalNoRef
  alt="Severity levels assigned by the technical edit"
  headers={[[{ value: 'Severity' }, { value: 'Meaning' }, { value: 'Author response' }]]}
  columns={[
    [
      '🔴 Must fix',
      '🟡 Should fix',
    ...
    [
      'Errors or accessibility violations (broken Section 508 compliance, factual grammar errors)',
      'Clarity, consistency, or readability issues',
      'Stylistic improvements the author may accept or decline',
    ],
    [
      'Required',
      'Required',
      'Author discretion — replying "leaving as-is" is a valid response',
    ],
  ]}
/>
...
Changes to the prompt — for example, adding a new style rule the AI has been missing — are proposed via a PR against [`.github/ai-review/technical-editor-prompt.md`](https://github.com/USACE-RMC/RMC-Software-Documentation/blob/main/.github/ai-review/technical-editor-prompt.md) with a site administrator tagged.
