...
At this point, the document has completed its preceding human stages. Technical editing is manually initiated. Review a new document in full; for a revision, review changed content with enough surrounding context to assess consistency, editorial quality, and accessibility.
...
<ProcessList
  items={[
    ...
    { title: 'When the technical edit has actually finished, an administrator records /review complete editor.' },
...
A technical edit can produce many inline comments. Batch related replies from **Files changed** with **Start a review** to reduce separate submissions and notification noise. Exact GitHub notification delivery depends on each user's settings.
:::

An author checkbox or label does not advance the stage. Any administrator may record `/review complete editor`; completed stages persist until an administrator explicitly uses `/review restart editor`. New-document Content PRs then merge as drafts before separate Director review. Major and minor revisions need no Director review.
