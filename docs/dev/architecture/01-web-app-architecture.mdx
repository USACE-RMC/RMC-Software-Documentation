...
<TableVerticalNoRef
  alt="Result object fields"
  headers={[[{ value: 'Field' }, { value: 'Purpose' }]]}
  columns={[
    ['status', 'inputs', 'warnings', 'errors', 'canRun', 'data', 'resultsCleared'],
    ['Overall result: success, incomplete (missing inputs), or error', 'Missing required inputs preventing analysis', 'Issues that don\'t prevent analysis but should be noted', 'Fatal issues that prevented the analysis', 'Whether the analysis can execute with current inputs', 'The response payload (structure varies by endpoint)', 'Whether previous results were invalidated'],
  ]}
/>
