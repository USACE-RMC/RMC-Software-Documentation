...
<TableVerticalNoRef
  alt="Key properties of calculation libraries"
  headers={[[{ value: 'Property' }, { value: 'Description' }]]}
  columns={[
    ['Stateless', 'Pure', 'Testable', 'Reusable'],
    ['Same inputs always produce same outputs', 'No side effects (no database, no HTTP, no file I/O)', 'Can be validated against known reference outputs', 'Multiple applications can consume the same library'],
  ]}
/>
...
<TableVerticalNoRef
  alt="HTTP API vs direct library comparison"
  headers={[[{ value: 'Factor' }, { value: 'HTTP API (Web Apps)' }, { value: 'Direct Library (Desktop/CLI)' }]]}
  columns={[
    ['Latency', 'Type safety', 'Deployment', 'Scaling', 'Debugging', 'Isolation'],
    ['Milliseconds (network)', 'Partial (runtime)', 'Multiple services', 'Scale calculations independently', 'Distributed tracing', 'Full resource isolation'],
    ['Microseconds', 'Full (compile-time)', 'Single process', 'Scale whole app', 'Single debugger', 'Shared process'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Test tolerances"
  headers={[[{ value: 'Calculation Type' }, { value: 'Tolerance' }, { value: 'Reason' }]]}
  columns={[
    ['Deterministic', 'Monte Carlo (same seed)', 'Monte Carlo (statistics)', 'Interpolation/lookup'],
    ['1e-10', '1e-10', '2-5%', '1e-12'],
    ['Near machine precision', 'Should be identical', 'Sampling variance', 'Should be exact'],
  ]}
/>
