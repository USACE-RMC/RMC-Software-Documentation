...
<TableVerticalNoRef
  ...
  columns={[
    ['React', 'Vite', 'JavaScript', 'Tailwind CSS', 'React Router', 'USACE Groundwork', 'Chart.js', 'Lucide React', 'KaTeX'],
    ['latest', 'latest', 'ES2022+', 'latest', 'latest', 'latest', 'latest', 'latest', 'latest'],
    ['Component-based UI framework', 'Build tool and development server', 'Programming language (not TypeScript)', 'Utility-first CSS framework', 'Client-side routing', 'USACE design system components', 'Charts and data visualization', 'Icon library', 'Mathematical notation rendering'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Common React hooks"
  headers={[[{ value: 'Hook' }, { value: 'Purpose' }, { value: 'Example' }]]}
  columns={[
    ['useState', 'useEffect', 'useMemo', 'useCallback'],
    ['Store and update component state', 'Run code when component mounts or data changes', 'Cache expensive calculations', 'Cache function references'],
    ['const [value, setValue] = useState(0)', 'Fetch data when page loads', 'Transform large datasets', 'Prevent unnecessary re-renders'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Vite path aliases"
  headers={[[{ value: 'Alias' }, { value: 'Maps To' }, { value: 'Example Import' }]]}
  columns={[
    ['@api/*', '@components/*', '@pages/*', '@hooks/*', '@contexts/*', '@data/*'],
    ['src/api/', 'src/components/', 'src/pages/', 'src/hooks/', 'src/contexts/', 'src/data/'],
    ["import { get } from '@api/common/apiFetch'", "import MessagesBanner from '@components/common/feedback/MessagesBanner'", "import Pfm01Page from '@pages/screening/pfm-01/Pfm01Page'", "import { useDebounce } from '@hooks/common/useDebounce'", "import { useAuth } from '@contexts/auth/AuthContext'", "import { constants } from '@data/common/constants'"],
  ]}
/>
...
<TableVerticalNoRef
  alt="Why five layers"
  headers={[[{ value: 'Layer' }, { value: 'Responsibility' }, { value: 'Benefit' }]]}
  columns={[
    ['Page', 'Controller', 'Data Hook', 'API Module', 'HTTP Utility'],
    ['Layout and UI composition', 'Data transformation → vm', 'State, orchestration, debouncing', 'Domain-specific endpoints', 'Auth, JSON parsing, errors'],
    ['Easy to see page structure; UI testing', 'Stable UI contract; hides API complexity', 'Centralized logic; reusable across UIs', 'Encapsulated URLs; mockable in tests', 'DRY; consistent behavior everywhere'],
  ]}
/>
...
<TableVerticalNoRef
  ...
  headers={[[{ value: 'Guideline' }, { value: 'Description' }]]}
  columns={[
    ['Single responsibility', 'Props over internal state', 'Composition over configuration', 'Consistent naming', 'Tailwind for styling'],
    ['Each component does one thing well', 'Parent controls data, child renders it', 'Build complex UIs by combining simple components', 'PascalCase for components, camelCase for props', 'Use utility classes, avoid custom CSS files'],
  ]}
/>
...
<TableVerticalNoRef
  alt="State management patterns"
  headers={[[{ value: 'Pattern' }, { value: 'When to Use' }, { value: 'Example' }]]}
  columns={[
    ['useState', 'useContext', 'Custom hooks', 'URL state', 'React Query'],
    ['Local component state', 'State shared across component tree', 'Reusable stateful logic', 'State that should persist across refresh', 'Many similar CRUD/list data-fetching patterns'],
    ['Form inputs, toggle states', 'Authentication, theme, feature vm/actions', 'Data fetching, debouncing', 'Analysis ID, active tab', 'Dashboard lists, paginated queries'],
  ]}
/>
...
<TableVerticalNoRef
  alt="React Query vs manual hooks"
  headers={[[{ value: 'Approach' }, { value: 'When to Use' }]]}
  columns={[
    ['React Query (TanStack Query)', 'Manual hooks'],
    ['CRUD-heavy pages with many similar data-fetching patterns, list pages, paginated queries, polling. Provides automatic caching, deduplication, and background refetching.', 'Complex auto-save flows with debouncing, hash-based change detection, phase tracking, and coordinated save-then-calculate sequences. When the data flow is bespoke and tightly controlled.'],
  ]}
/>
