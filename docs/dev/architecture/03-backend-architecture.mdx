...
<TableVerticalNoRef
  ...
  headers={[[{ value: 'Technology' }, { value: 'Version' }, { value: 'Purpose' }]]}
  columns={[
    ['ASP.NET Core', 'Entity Framework Core', 'PostgreSQL', 'JWT Bearer', 'Serilog'],
    ['Latest', 'Latest', '15+', '-', '-'],
    ['Web API framework', 'Object-relational mapper (ORM)', 'Relational database', 'Authentication tokens', 'Structured logging'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Access modifiers"
  headers={[[{ value: 'Modifier' }, { value: 'Meaning' }]]}
  columns={[
    ['public', 'private', 'protected', 'internal'],
    ['Any code can access this member', 'Only code within the same class can access this member', 'Only the class and its derived classes can access this member', 'Only code within the same project can access this member'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Why Clean Architecture for RMC applications"
  headers={[[{ value: 'Requirement' }, { value: 'How Clean Architecture Helps' }]]}
  columns={[
    ['Life safety applications', 'Multiple teams', 'Long-term maintenance', 'Complex calculations', 'Regulatory compliance'],
    ['Developers can exhaustively unit test domain logic in complete isolation from databases and HTTP—critical calculations become verifiable', 'Clear project boundaries allow teams to own specific layers without conflicts', 'The dependency rule prevents architectural decay; inner layers remain stable as outer layers change', 'Engineering calculations live in the Domain layer, protected from infrastructure concerns', 'Separation makes it easier to audit and verify critical business logic'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Layer responsibilities"
  headers={[[{ value: 'Layer' }, { value: 'What Lives Here' }, { value: 'What It Does' }]]}
  columns={[
    ['Domain', 'Application', 'Infrastructure', 'API'],
    ['Entities, enums, business logic', 'Services, interfaces, DTOs', 'DbContext, repositories, typed HTTP clients', 'Controllers, middleware, Program.cs'],
    ['Defines data structures and core business rules', 'Coordinates operations, defines contracts for infrastructure', 'Implements data access and external integrations', 'Handles HTTP requests and responses'],
  ]}
/>
...
<TableVerticalNoRef
  alt="HTTP methods and routes"
  headers={[[{ value: 'Operation' }, { value: 'HTTP Method' }, { value: 'Route Example' }, { value: 'What It Does' }]]}
  columns={[
    ['Get one', 'Get many', 'Create', 'Update', 'Delete', 'Run action'],
    ['GET', 'GET', 'POST', 'PUT', 'DELETE', 'POST'],
    ['/api/analyses/123', '/api/analyses', '/api/analyses', '/api/analyses/123', '/api/analyses/123', '/api/analyses/123/calculate'],
    ['Retrieves a single analysis', 'Retrieves a list of analyses', 'Creates a new analysis', 'Updates an existing analysis', 'Deletes an analysis', 'Triggers a calculation'],
  ]}
/>
...
<TableVerticalNoRef
  alt="Log prefixes"
  headers={[[{ value: 'Prefix' }, { value: 'Usage' }]]}
  columns={[
    ['[API]', '[CALC]', '[VALID]', '[DB]', '[ERROR]'],
    ['HTTP request/response handling', 'Engineering calculations', 'Input validation', 'Database operations', 'Error conditions'],
  ]}
/>
...
<TableVerticalNoRef
  ...
  columns={[
    ['API', 'Async/Await', 'Attribute', 'Clean Architecture', 'Constructor', 'Controller', 'DbContext', 'Dependency Injection', 'DTO', 'Entity', 'Extension Method', 'Fluent API', 'Generic', 'Interface', 'JWT', 'Lambda Expression', 'Layer', 'Middleware', 'Namespace', 'NuGet Package', 'ORM', 'POCO', 'Property', 'Repository', 'Service'],
    ['Application Programming Interface—a way for programs to communicate, typically over HTTP', 'C# keywords for handling long-running operations without blocking', 'Metadata attached to code elements using [brackets]', 'A software design pattern where dependencies point inward toward business logic', 'A special method that runs when creating a new object', 'A class that handles HTTP requests in ASP.NET Core', "Entity Framework Core's class for database interaction", 'A pattern where objects receive their dependencies from outside', 'Data Transfer Object—a simple object for carrying data between layers', 'A class representing a business object, often mapped to a database table', 'A method that adds functionality to an existing type', "EF Core's method-chaining approach to configure entity mappings", 'A type that works with different types, specified in angle brackets &lt;&gt;', 'A contract defining what methods a class must implement', 'JSON Web Token—a signed token containing user identity information', 'A shorthand for writing small functions using =&gt;', 'A logical grouping of code with specific responsibilities', 'Code that runs for every HTTP request in the pipeline', 'A way to organize code and prevent naming conflicts', 'A library published for use in .NET projects', 'Object-Relational Mapper—a tool that maps database records to objects', 'Plain Old CLR Object—a simple class with no framework dependencies', 'A class member that encapsulates a field with get/set accessors', 'A class that handles data access for a specific entity type', 'A class containing business or application logic'],
  ]}
/>
