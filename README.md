...
- `draftDocs.js`: This file aggregates all document entries from each navigation/category file and exports an array (`draftDocs`) containing the base paths of all documents marked as `draft: true`. This array is used by theme components to determine if a documentation page should display a DRAFT watermark. To add new categories, import their docs arrays and include them in `allDocs`.

- `reportIdMap.js`: This auto-generated file exports a mapping of documentation base paths (including version) to unique report IDs. It is used to quickly look up a report's identifier based on its path. Do not edit this file manually; it is regenerated as needed.
